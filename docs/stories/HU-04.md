...
8. **CA-04.08 (desasignar) elimina todo el slot con sus alternativas.** La operación busca el `slot` del ejercicio via `getSlotForExercise()` y elimina todas las filas de `plan_assignment` de ese slot via `deleteBySlot()`. Si el ejercicio no tiene slot (fallback), se elimina solo la fila individual. Los ejercicios permanecen en `exercise` (Diccionario) y los registros históricos en `exercise_set` no se afectan (FK no cascadea). La desasignación no afecta otras versiones ni las sesiones pasadas. **No se permite desasignar ejercicios mientras haya una sesión activa de esa versión** (`hasActiveSessionForVersion` guard en `UnassignExerciseFromVersionUseCase`).
