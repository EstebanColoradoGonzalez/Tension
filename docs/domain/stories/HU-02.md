# Historia de Usuario: `HU-02` — Consultar historial de peso corporal
...
- **Entidades Afectadas:** `WeightRecordEntity` / `WeightRecord` (lectura) (Referencia a `docs/architecture/domain_and_state_model.md`).
- **Mutaciones de Estado:** Ninguna — esta historia es de solo lectura. CA-02.01 está cubierto por HU-01 (`UpdateWeightUseCase` inserta en `weight_record` append-only).

### 4.2. Contrato de Interfaces (Triggers / API)

- **Trigger / Endpoint consumido:** `Navegación C1 → C2` — enlace "Ver historial de peso →" de `ProfileScreen` (Referencia a `docs/architecture/interfaces_contract.md`).
- **Payload requerido:** Ninguno — lectura reactiva via `Flow<List<WeightRecord>>`.

### 4.3. UI / Assets

- **Componente Visual:** `C2 — WeightHistoryScreen` (reemplaza stub creado en HU-01). `BottomNavigationBar` requiere ajuste de selección para rutas hijas del flujo perfil.
