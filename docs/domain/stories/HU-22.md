# HU-22 — Preview de sesión sin iniciar y cronómetro para ejercicios por tiempo
...
|--------------------|-----------------------------------------------------------------------|
| **ID**             | HU-22                                                                 |
| **Título**         | Preview de sesión sin iniciar y cronómetro para ejercicios por tiempo |
| **Épica**          | Experiencia de Entrenamiento                                          |
| **Requisitos**     | RF12 (extensión), RF32 (extensión), RNF02                            |
| **Prioridad**      | Media                                                                 |
| **Estado**         | Implementada ✅                                                       |
| **Arquitecto**     | Analizada — 2026-03-05 (auditoría: 15 defectos corregidos)           |
| **Developer**      | Refinada — 2026-03-05                                                 |
| **Implementación** | Completada — 2026-03-05                                               |
| **Validación**     | Esteban — 2026-03-05                                                  |
| **Migración DB**   | No requerida (sin cambio de schema)                                   |
| **Build**          | BUILD SUCCESSFUL — auditoría 12 PASS, 2 issues corregidos            |
...
| Historia | Relación |
|----------|----------|
| **HU-21** (Reestructuración del plan, completada) | El preview debe mostrar el orden sugerido de ejecución (CA-22.04) y la indicación "fuera del gym" para abdomen (CA-22.05). `sort_order` en `plan_assignment`, badge derivado de `isBodyweight + moduleCode == "A"`. |
| **HU-05** (Inicio de sesión) | `startSession()` / `StartSessionUseCase`. Crash recovery. Carga objetivo (`prescribedLoadKg` via `exercise_progression`). CA-05.07 y CA-05.08 replicados en el preview. |
| **HU-06** (Registro de serie) | `RegisterSetScreen` (E2). Campo `reps` almacena segundos para isométricos. El cronómetro modifica esta pantalla. |
| **HU-07** (Sustitución de ejercicios) | Sustitutos no tienen `plan_assignment` → `COALESCE(pa.reps, '8-12')` → fallback del cronómetro si isométrico. |
| **HU-08** (Cross-cutting isométrico) | Peso fijo en 0 Kg, texto "Segundos sostenidos", referencia "30-45 seg". El cronómetro reemplaza el campo manual. |
| **HU-14** (Protocolo de descarga) | Preview deload-aware: 7 variantes de `loadDisplayText`, `DeloadLoadRule`, indicador visual deload, 6 sesiones, reinicio al 90%. |
| **HU-16** (Reestructuración DB con `sort_order` y PlanSeeder) | Formato `'30-45_SEC'` en `plan_assignment.reps` — base del `RepsRangeParser`. |

### Requisitos técnicos

- **Sin nueva migración de BD:** Todo el Preview opera con queries de solo lectura sobre tablas existentes (`plan_assignment`, `exercise`, `exercise_progression`, `module`). El cronómetro almacena en el mismo campo `exercise_set.reps` con el mismo formato entero.
- **Sin nuevas dependencias:** `SystemClock.elapsedRealtime()` pertenece al SDK de Android (`android.os`).
- **Parámetros de navegación del Preview:** `moduleVersionId`, `moduleCode`, `versionNumber` — los 3 valores evitan un query extra para resolver el header.

### Historias relacionadas (consumidores de CA)

| CA | Historia relacionada | Descripción |
|----|----------------------|-------------|
| CA-22.04 | HU-21 | Orden sugerido `sort_order` en `plan_assignment` |
| CA-22.05 | HU-21 | Badge "Fuera del gym": `isBodyweight && moduleCode == "A"` |
| CA-22.07 | HU-09 | Cierre de sesión → `evaluateProgression()` actualiza `prescribed_load_kg` |
| CA-22.14 | HU-08 | Peso fijo en 0 Kg para isométricos |
| CA-22.19 | HU-10, HU-11, HU-15 | Motor de progresión, Doble Umbral, KPIs operan sin modificación |
...
<!-- BASE: Análisis Arquitectónico (Arquitecto) - Ver §5                        -->
<!-- FECHA: 2026-03-05                                                            -->
