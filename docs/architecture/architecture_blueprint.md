...
- **Decisión:** `TensionDatabase.version` sube sin migración y **sin** `fallbackToDestructiveMigration()`. Aplicado a v17 (`week_day`, `daily_routine_override`), a v18 (`day_skip`), a v19 (`tree_state`, HU-37) y a **v20** (`exercise_equipment`, `exercise_set.equipment_type_id` y la retirada de `exercise.equipment_type_id`, HU-39). Las migraciones históricas se conservan intactas: describen el esquema de su época. Las de `16→17`, `17→18` y `18→19`, ausentes durante tres versiones, se repusieron después de dejar la aplicación incapaz de abrir cualquier base existente — y de ahí sale la forma que toma la guarda.
...
- **Consecuencias:** Excepción documentada a RNF19, limitada a la fase beta. Una base anterior falla al abrir con la excepción de Room, que es la señal de reinstalar; la app nunca borra datos por su cuenta. Con HU-39 el historial de sesiones anterior **se pierde**, y queda declarado como consecuencia aceptada. Los respaldos en formatos 8 a 12 dejan de poder restaurarse. Ningún test ejerce el arranque en v20 porque no hay migración que probar; lo que se verifica es la semilla sobre instalación fresca (`CatalogoSembradoTest`, `ExerciseCatalogTest` y validación manual). **El criterio lo heredan las cuatro historias hermanas de HU-39** (`HU-40` … `HU-43`): cada una extiende el esquema y se resuelve del mismo modo.
