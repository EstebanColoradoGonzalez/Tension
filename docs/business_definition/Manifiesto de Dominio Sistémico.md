# Manifiesto de Dominio Sistémico (MDS)

## 1. Identidad y Propósito Superior (El "Para qué")

### Misión del Sistema

Transformar el registro disciplinado de datos de entrenamiento de fuerza — carga, repeticiones, intensidad percibida (RIR) y adherencia — en un modelo de decisiones que garantice la **sobrecarga progresiva continua** del ejecutante, eliminando la subjetividad, el estancamiento y la improvisación del proceso de hipertrofia.

El problema fundamental que este sistema resuelve es el siguiente: un ejecutante que entrena sin un mecanismo formal de análisis de sus datos opera a ciegas. Puede registrar números en una libreta durante años y aún así estancarse, porque el dato crudo sin interpretación no genera adaptación. Este sistema existe para cerrar esa brecha entre **dato capturado** y **decisión informada**.
...
El sistema opera dentro de un entorno gobernado por leyes fisiológicas y factores externos que influyen directamente en los resultados, pero que **no están bajo su control**:
...
## 3. Actores y Roles (El "Quién")

> *En este sistema, los actores no son "usuarios de software" sino participantes funcionales del proceso de entrenamiento. Al ser un sistema de uso personal, los roles pueden converger en una misma persona, pero se formalizan por separado porque representan responsabilidades distintas.*

### Participantes Primarios

#### El Ejecutante

Es la persona que realiza el entrenamiento físico y genera los datos del sistema. Es simultáneamente el operador y el beneficiario.
...
Es el conjunto de reglas, cálculos y estructuras que procesan los datos del Ejecutante y producen decisiones. No es una persona, pero cumple un rol activo dentro del proceso: **analizar, prescribir y alertar**.
...
- Preservar la integridad y trazabilidad completa del historial de datos.

### Beneficiarios

#### El Ejecutante (en su rol de beneficiario)

El mismo actor que genera los datos es quien recibe el valor. Obtiene:

- Claridad operativa sobre qué hacer en cada sesión.
- Evidencia objetiva de su progresión a lo largo del tiempo.
- Autonomía para tomar decisiones informadas sobre su entrenamiento sin dependencia de terceros.

### Interventores

En este sistema no existen interventores regulatorios externos. Sin embargo, se reconoce un rol opcional:

#### Asesor Externo (opcional)

Un entrenador, fisioterapeuta o profesional de la salud que podría consultar los datos históricos del sistema para emitir recomendaciones complementarias (ajuste de plan por lesión, revisión de técnica, modificación nutricional).

**Responsabilidades:**

- Interpretar los datos del sistema desde una perspectiva clínica o técnica especializada.
- Sugerir modificaciones al Plan de Entrenamiento o al Diccionario de Ejercicios.
- Este rol **no modifica directamente** las reglas ni los datos del sistema; solo emite recomendaciones que el Ejecutante decide integrar o no.

### Resumen de Roles

| **Actor** | **Tipo** | **Acción Principal** | **Genera Datos** | **Consume Datos** |
| --- | --- | --- | --- | --- |
| Ejecutante | Primario / Beneficiario | Entrena, registra, consulta | Sí | Sí |
| Sistema | Primario (automatizado) | Calcula, prescribe, alerta | Sí (derivados) | Sí |
| Asesor Externo | Interventor (opcional) | Revisa, recomienda | No | Sí |

## 4. Ontología del Dominio (El "Qué" - Los Conceptos)

### A. Glosario de Términos (Lenguaje del Sistema)

> *Definición precisa de cada concepto que el sistema manipula. Todo actor que interactúe con el sistema debe compartir este vocabulario sin ambigüedad.*
...
- **Tonelaje:** El volumen total de carga movida en un contexto dado. Se calcula como $Peso \times Repeticiones \times Series$. Puede medirse por ejercicio, por sesión o por grupo muscular.
...
Para que el sistema tome decisiones, clasifica los ejercicios según su función biomecánica:

| **Categoría** | **Función Principal** | **Ejemplo en el Plan** |
| --- | --- | --- |
| **Multiarticulares (Compuestos)** | Gran demanda del SNC, permiten cargas altas. Involucran dos o más articulaciones. | Press de Banca, Sentadilla, Remo con Inclinación. |
| **Aislamiento (Accesorios)** | Enfoque en un músculo específico, bajo estrés sistémico. Involucran una sola articulación. | Curl de Bíceps, Extensión de Cuádriceps, Cruce en Polea Alta. |
| **Tracción (Pull)** | Movimientos que acercan la carga al cuerpo. | Tiro de Dorsales (Agarre ancho), Remo con un solo brazo doblado. |
| **Empuje (Push)** | Movimientos que alejan la carga del cuerpo. | Press de Banca, Press de Banca Inclinada, Press de Elevación Sentado. |
| **Cadena Posterior** | Movimientos que enfatizan glúteos, isquiotibiales y espalda baja. | Empuje de Cadera, Curl Femoral Tumbado, Zancada hacia atrás. |
| **Cadena Anterior** | Movimientos que enfatizan cuádriceps y flexores de cadera. | Sentadilla, Extensión de Cuádriceps, Press de Pierna. |

> *Nota: Un ejercicio puede pertenecer a más de una categoría simultáneamente. Por ejemplo, la Sentadilla es Multiarticular, de Empuje y de Cadena Anterior. Esta clasificación múltiple permite al sistema analizar la distribución del estímulo desde distintos ejes.*

## 5. Dinámica del Sistema (El "Cómo" - Procesos de Negocio)

> *Esta sección describe cómo fluye el valor dentro del sistema: desde la entrada del ejecutante al gimnasio hasta la decisión informada que optimiza su siguiente sesión. Los procesos aquí descritos son independientes de la herramienta que los implemente.*

### A. Flujos de Valor Principales

#### Flujo 1: Ejecución y Registro de una Sesión
...
3. **Ejecución del entrenamiento.** El Ejecutante realiza los ejercicios prescritos **en el orden que decida** según la disponibilidad de equipo y las circunstancias del gimnasio, respetando las 4 series y el rango de 8-12 repeticiones (o la condición especial: fallo técnico, isométrico en segundos). El Plan de Entrenamiento lista los ejercicios, no los ordena.
...
3. **Clasificación del resultado.** Cada ejercicio en la sesión recibe una clasificación de progresión:
   ...
   - **Regresión:** Disminuyó carga o repeticiones, o el RIR aumentó significativamente con la misma carga (señal de fatiga acumulada).
4. **Emisión de señales.** Basándose en las Reglas de Negocio (§6), el sistema genera recomendaciones: subir carga en la próxima sesión, mantener, solicitar descarga, o rotar versión.

#### Flujo 3: Gestión del Mesociclo y Descarga
...
#### Flujo 4: Detección y Resolución de Mesetas
...
Los siguientes sucesos disparan una acción dentro del sistema:
...
#### Ciclo de vida de un Ejercicio (dentro de una sesión)

```
[No Iniciado] → [En Ejecución] → [Completado]
```
...
```
[Sin Historial] → [En Progresión] ⇄ [En Meseta] → [En Descarga] → [En Progresión]
```

- **Sin Historial:** El ejercicio se ha ejecutado 0 o 1 vez. No hay base de comparación.
- **En Progresión:** El ejercicio muestra progresión positiva entre sesiones consecutivas.
- **En Meseta:** 3+ sesiones sin progresión. El sistema ha emitido alerta.
- **En Descarga:** El ejercicio está en una semana de descarga planificada. No se espera progresión.

## 6. Reglas de Negocio y Restricciones (El "Bajo qué leyes")

> *Las leyes que gobiernan el comportamiento del sistema. Son inamovibles independientemente de si la implementación es una app, una hoja de cálculo o un cuaderno de papel. Violarlas produce datos corruptos o decisiones incorrectas.*

### A. Reglas de Decisión

#### Regla 1: Progresión de Carga (Doble Umbral)
...
Si el Ejecutante **no** cumple las condiciones de la Regla 1, la carga se mantiene para la próxima sesión. El objetivo es progresar en repeticiones dentro del rango prescrito (de 8 hacia 12) manteniendo o reduciendo el RIR.

**Acción:** El sistema prescribe la misma carga y alienta al Ejecutante a buscar 1 repetición más por serie o un RIR más bajo con la misma técnica.
...
Se identifica **regresión** cuando en una sesión, comparada con la anterior del mismo ejercicio:
...
Cuando el sistema prescribe una descarga, los parámetros son:
...
#### Regla 6: Progresión en Ejercicios de Peso Corporal
...
#### Regla 7: Progresión en Ejercicios Isométricos
...
11. **El orden de ejecución de ejercicios dentro de una sesión es libre.** El Plan de Entrenamiento lista los ejercicios de cada versión pero **no prescribe una secuencia obligatoria**. El Ejecutante decide en qué orden ejecutarlos según la disponibilidad de estaciones en el gimnasio y sus circunstancias. Esta flexibilidad no afecta la validez de los datos ni los cálculos del sistema.
...
Normas que guían la operación del sistema pero que admiten excepciones justificadas por el Ejecutante:

1. **Política de frecuencia mínima:** El sistema está diseñado para un mínimo de 4 y un máximo de 6 sesiones semanales. Menos de 3 sesiones en una semana natural debería generar una alerta de baja adherencia (informativa, no bloqueante).
2. **Política de registro inmediato:** Los datos deben registrarse durante o inmediatamente después de cada serie. El registro retrospectivo (al día siguiente, de memoria) compromete la precisión del RIR y las repeticiones.
3. **Política de honestidad en el RIR:** El RIR es una medida subjetiva. El sistema confía en la honestidad del Ejecutante. Inflar o deflactar el RIR sistemáticamente corrompe las señales de progresión y hace que las reglas de decisión fallen.
4. **Política de técnica sobre carga:** Si el Ejecutante siente que la técnica se degrada antes de llegar al RIR reportado, debe reducir la carga. La integridad articular y la seguridad a largo plazo prevalecen sobre la progresión numérica de corto plazo.
5. **Política de duración de sesión:** No se prescribe un tiempo máximo de sesión. Sin embargo, si una sesión excede consistentemente las 2 horas, puede indicar descansos excesivamente largos entre series, lo cual diluye la densidad del estímulo.

## 7. Medidores de Salud y Desempeño (El "Cuánto")

> *Estos indicadores permiten al Ejecutante evaluar si el sistema está cumpliendo su propósito. No son métricas técnicas de software, sino medidores de negocio que responden a la pregunta: "¿Estoy progresando?".*

### A. Indicadores Clave de Desempeño (KPIs de Negocio)

#### KPI 1: Tasa de Progresión por Ejercicio

**Definición:** Porcentaje de sesiones en las que un ejercicio registró progresión positiva (aumento de carga o repeticiones) respecto al total de sesiones del mismo ejercicio en un período dado.

$$
\text{Tasa de Progresión} = \frac{\text{Sesiones con progresión positiva}}{\text{Total de sesiones del ejercicio}} \times 100
$$

**Interpretación:**

- **≥ 60%:** El ejercicio progresa de forma saludable. El estímulo y la recuperación están equilibrados.
- **40% - 59%:** Progresión moderada. Esperable en ejercicios maduros (carga ya alta) o después de varios mesociclos.
- **< 40%:** El ejercicio está estancado o en regresión. Requiere intervención (ver Regla 3, §6).

**Periodicidad de evaluación:** Cada 4 semanas o al cierre de un mesociclo.

#### KPI 2: Tonelaje Acumulado por Grupo Muscular
...
- **Tendencia ascendente sostenida:** El grupo muscular está recibiendo estímulo creciente. Señal positiva.
- **Estabilización:** El tonelaje se mantiene pero no crece. Puede indicar que se está en un rango de mantenimiento o que se necesita intervención.
- **Caída sostenida (≥ 2 microciclos):** Señal de fatiga acumulada o pérdida de adherencia. Evaluar necesidad de descarga.
...
**Definición:** La relación entre las sesiones efectivamente completadas y las sesiones esperadas según la frecuencia planificada, medida por semana natural.

$$
\text{Adherencia} = \frac{\text{Sesiones completadas en la semana}}{\text{Sesiones planificadas}} \times 100
$$

Donde "sesiones planificadas" se define como el objetivo de frecuencia del Ejecutante (entre 4 y 6).

**Interpretación:**

- **≥ 80%:** Adherencia alta. El sistema opera con datos suficientes para generar señales confiables.
- **60% - 79%:** Adherencia aceptable. Las señales del sistema son válidas pero pueden tener menor resolución temporal.
- **< 60%:** Adherencia baja. Las comparaciones entre sesiones pierden validez porque el tiempo entre ellas es excesivo (la recuperación altera las condiciones). El sistema emite una alerta informativa.

**Periodicidad de evaluación:** Semanal.

#### KPI 5: Velocidad de Progresión de Carga

**Definición:** El incremento promedio de carga (en Kg) por ejercicio entre la primera sesión registrada y la más reciente, normalizado por el número de sesiones intermedias.

$$
\text{Velocidad} = \frac{Peso_{\text{actual}} - Peso_{\text{inicial}}}{\text{Número de sesiones}}
$$

**Interpretación:**

- Este indicador varía enormemente entre ejercicios y nivel de experiencia. No tiene umbrales universales. Su valor está en la **comparación relativa**: permite al Ejecutante identificar qué ejercicios progresan más rápido que otros, y por tanto, dónde el cuerpo responde mejor o peor a los estímulos.
- Una velocidad de 0 durante un período prolongado confirma una meseta activa.

**Periodicidad de evaluación:** Cada mesociclo (4-8 semanas).
...
### B. Umbrales de Tolerancia

Los siguientes umbrales definen cuándo el sistema considera que algo requiere atención inmediata:
