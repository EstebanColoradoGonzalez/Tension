...
- **Decisión:** `TensionDatabase.version` sube sin migración y **sin** `fallbackToDestructiveMigration()`. Aplicado a v17 (`week_day`, `daily_routine_override`) y a v18 (`day_skip`). Las migraciones históricas `MIGRATION_6_7` … `MIGRATION_15_16` se conservan intactas: describen el esquema de su época. El formato de respaldo sube a `SCHEMA_VERSION = 11`: v10 incorporó `week_day` y `daily_routine_override` —omitirlas perdería la relación día → rutina en cada restauración— y v11 añade `day_skip`.
