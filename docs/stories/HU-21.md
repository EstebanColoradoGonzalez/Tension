...
- **`PlanAssignmentDao` — query `getPreviewByModuleVersionId()` (Modificación):** Nuevo query que extiende el existente `getDetailsByModuleVersionId()` añadiendo `LEFT JOIN exercise_progression` para obtener `prescribed_load_kg`. La consulta ya resuelve `sort_order` (HU-20), `muscle_zones`, `equipment_type`, `isBodyweight`, `isIsometric`. Solo falta `prescribedLoadKg`. Nuevo DTO: `SessionPreviewExerciseDto` con 10 campos.
  ...
  - Especificaciones: Query SQL:
    ...
    ```
    Patrón idéntico al que `getBySessionIdWithDetails()` en `SessionExerciseDao` ya usa con `LEFT JOIN exercise_progression`.

- **`SessionPreviewExercise` — Modelo de dominio (Nuevo):** Modelo read-only que transporta los datos del preview desde la capa de datos hasta la UI. Campos: `exerciseId`, `exerciseName`, `moduleCode`, `equipmentTypeName`, `muscleZones`, `sets`, `reps`, `isBodyweight`, `isIsometric`, `prescribedLoadKg`.
...
- **`SessionPreviewViewModel` (Nuevo):** Inyecta `GetSessionPreviewUseCase` y `StartSessionUseCase`. Estado via `StateFlow<SessionPreviewUiState>` con `moduleCode`, `versionNumber`, `moduleVersionId`, `exercises`, `isLoading`. Acción `startSession()` → llama `StartSessionUseCase(moduleVersionId)` → emite evento de navegación a ActiveSession via `SharedFlow`. `moduleVersionId` recibido desde `SavedStateHandle`.
  - Ubicación: `ui/session/SessionPreviewViewModel.kt`

- **`SessionPreviewScreen` (Nuevo):** Pantalla Compose read-only. Header "Módulo {code} — V{n}". `LazyColumn` con ejercicios en sort_order. Cada item: nombre, zona muscular, equipamiento, sets×reps, carga objetivo (o "Primera sesión" si `prescribedLoadKg == null`). Ejercicios bodyweight del módulo A: badge "Fuera del gym" (CA-21.05, patrón HU-20). Botón CTA "Iniciar Sesión" → crea sesión y navega a ActiveSession. TopAppBar con botón atrás → regresa a Home sin consecuencias (CA-21.03). Pantalla separada (no bottom sheet) porque la lista de 6-8 ejercicios con cargas excede el espacio de un bottom sheet, y permite ruta propia (ADR-07).
  - Ubicación: `ui/session/SessionPreviewScreen.kt`

- **`NavigationRoutes` / `TensionNavHost` (Modificación):** Nueva ruta `SESSION_PREVIEW = "session-preview/{moduleVersionId}"`. Nuevo `composable` en NavHost con `SessionPreviewScreen`. HomeScreen: nueva callback `onNavigateToPreview(moduleVersionId)`. Modificación de `NextSessionCard`: el card se hace clickable → navega al preview. Botón "Iniciar Sesión" se mantiene como acción rápida alternativa (usuario elige: tap card → preview, o tap botón → inicio directo).
  - Nivel de cambio: Menor
...
- **`SetExerciseInfo` DTO en `SessionExerciseDao` (Modificación):** Agregar campo `reps: String` al DTO existente. Modificar query `getExerciseInfoForSet`: añadir `COALESCE(pa.reps, '8-12') AS reps` al SELECT. La cláusula `LEFT JOIN plan_assignment` ya existe en la query.
  - Nivel de cambio: Menor

- **`RegisterSetInfo` — Modelo de dominio (Modificación):** Agregar campo `prescribedReps: String`. Transporta el rango prescrito desde la capa de datos hasta el ViewModel. Modificar `getRegisterSetInfo()` en `SessionRepositoryImpl`: mapear `info.reps` → `prescribedReps`.
  - Nivel de cambio: Menor

- **`RegisterSetUiState` (Modificación):** Nuevos campos para el estado del timer: `timerState: TimerState` (enum: IDLE, RUNNING, STOPPED), `timerSeconds: Int`, `minSeconds: Int?`, `maxSeconds: Int?`, `showChronometer: Boolean` (derivado de `isIsometric && minSeconds != null`). Actualizar `isConfirmEnabled`: cuando `showChronometer`, se habilita solo si `timerState == STOPPED && selectedRir != null`.
  - Nivel de cambio: Mayor

- **`RegisterSetViewModel` (Modificación):** En `loadInfo()`: parsear `prescribedReps` con `RepsRangeParser`. Si `isIsometric && range.isSeconds` → activar cronómetro con `minSeconds=range.min`, `maxSeconds=range.max`. Si `isIsometric && !range.isSeconds` → fallback `minSeconds=30`, `maxSeconds=60` (CA-21.16). Nuevos métodos `onStartTimer()` y `onStopTimer()`. Timer implementado con `viewModelScope.launch` + `SystemClock.elapsedRealtime()` en loop de ~100ms para conteo preciso. `elapsedRealtime()` no se pausa cuando la app va a background → resuelve CA-21.17. Auto-stop cuando `timerSeconds >= maxSeconds` (CA-21.09). Al detener: asigna `reps = elapsed.toString()`.
  - Nivel de cambio: Mayor

- **`IsometricChronometer` — Composable (Nuevo):** Componente visual con display grande de segundos (tipografía `headlineLarge`), `CircularProgressIndicator` determinado (`progress = seconds / maxSeconds`), colores semánticos: `Error` (rojo Material3) pre-mínimo → `Primary` (verde) en rango → auto-stop al máximo. Botón "Iniciar" → muta a "Detener" cuando en ejecución. Texto de soporte: "Rango: {min}–{max} seg". Indicación bajo el rango si se detiene antes del mínimo (CA-21.11).
...
1. **`RepsRangeParser` + tests unitarios** — Utilidad de parsing del formato `'30-45_SEC'`. Sin dependencias externas. Base para el cronómetro y reutilizable en preview.
   - Dependencias: Ninguna

2. **Capa de datos del Preview** — `SessionPreviewExerciseDto`, query `getPreviewByModuleVersionId()` en `PlanAssignmentDao`, método `getSessionPreviewExercises()` en `SessionRepository`/`SessionRepositoryImpl`.
...
4. **UI del Preview** — `SessionPreviewViewModel`, `SessionPreviewUiState`, `SessionPreviewScreen`, ruta `session-preview/{moduleVersionId}` en `TensionNavHost`, modificación de `NextSessionCard` en `HomeScreen`.
   - Dependencias: Hito 3, reutiliza `StartSessionUseCase` existente

5. **Extensión del modelo de datos para cronómetro** — `SetExerciseInfo.reps` (DTO), `RegisterSetInfo.prescribedReps` (modelo de dominio), query `getExerciseInfoForSet` modificada, mapping en `SessionRepositoryImpl.getRegisterSetInfo()`.
   - Dependencias: Ninguna nueva (componentes existentes)

6. **Cronómetro — ViewModel + Composable** — `TimerState` enum, campos de timer en `RegisterSetUiState`, lógica de timer en `RegisterSetViewModel`, componente `IsometricChronometer`, integración condicional en `RegisterSetScreen`.
   - Dependencias: Hito 1, Hito 5
...
| --- | --- | --- |
| `NextSession` (modelo) | Solo 3 campos (moduleCode, versionNumber, moduleVersionId) | Sin cambio — el preview usa su propia ruta con `moduleVersionId` como parámetro |
...
| `SessionExerciseDao.getExerciseInfoForSet()` | 6 campos, LEFT JOIN plan_assignment, no incluye `reps` | Modificación menor — agregar campo `reps` al SELECT y al DTO |
| `RegisterSetInfo` (modelo) | 11 campos, no incluye prescribedReps | Modificación menor — agregar `prescribedReps: String` |
| `RegisterSetViewModel` | Maneja isIsometric (weight=0, reps manual) | Modificación mayor — timer state, parseo de reps, start/stop |
...
| `RegisterSetUiState` | 16 campos, `isConfirmEnabled` computed | Modificación mayor — 5 nuevos campos, `isConfirmEnabled` actualizado |
...
| `TensionNavHost` | 12 rutas, bottom bar hidden para session routes | Modificación menor — +1 ruta |
...
```text
HomeScreen (tap card) → navigate("session-preview/$moduleVersionId")
  → SessionPreviewViewModel.init(moduleVersionId)
    → GetSessionPreviewUseCase(moduleVersionId)
      → SessionRepository.getSessionPreviewExercises(moduleVersionId)
        → PlanAssignmentDao.getPreviewByModuleVersionId()  [READ-ONLY]
  → SessionPreviewScreen renders exercises
...
```text
RegisterSetScreen (ejercicio isométrico)
  → RegisterSetViewModel.loadInfo(sessionExerciseId)
    → GetRegisterSetInfoUseCase → RegisterSetInfo (incluye prescribedReps="30-45_SEC")
  ...
  → IsometricChronometer renders
    → User: "Iniciar" → onStartTimer() → coroutine con elapsedRealtime
      → Color: Error (0-29s) → Primary (30-45s)
        → At 45s: auto-stop → reps = "45"
        → OR user stops at 33s → reps = "33"
...
| --- | --- | --- |
| Inconsistencia de cargas entre preview y sesión real (CA-21.07) | Si otra sesión se cierra entre preview y start, `prescribed_load_kg` puede cambiar | Ambos leen del mismo `exercise_progression` via LEFT JOIN. Se actualiza solo en `evaluateProgression()` al cerrar sesión. CA-21.07 contempla el escenario explícitamente |
...
| Ejercicio isométrico custom sin rango `_SEC` (CA-21.16) | Ejercicio custom marcado isométrico pero con reps `"8-12"` | Fallback: si `isIsometric && !reps.endsWith("_SEC")` → `minSeconds=30, maxSeconds=60` con indicación de rango por defecto |

### Notas Técnicas (Si aplica)

- **No se requiere nueva migración de base de datos.** Todo el Preview opera con un query de solo lectura sobre tablas existentes (`plan_assignment`, `exercise`, `exercise_progression`). El cronómetro almacena datos en el mismo campo `exercise_set.reps` con el mismo formato entero.
- **`moduleVersionId` como parámetro de navegación** para el Preview es suficiente. No se necesita pasar `moduleCode` ni `versionNumber` — el ViewModel puede derivarlos del query o recibirlos como parámetros adicionales de la ruta para evitar un query extra.
...
- **Consistencia del preview con la sesión real** (CA-21.07): Dado que ambos leen de `exercise_progression.prescribed_load_kg` vía LEFT JOIN, la consistencia está garantizada si no hay cierre de sesión intermedio. El CA ya contempla el caso excepcional.

### Referencias y Validación

**Documentación consultada:**
- Visión del Producto — §4.5 (sesión pertenece al usuario), §4.8 (fricción → cero)
- Requerimientos — RF12 (preview sesión), RF32 (isométricos 30-45s), RNF02 (max 3 toques)
- Arquitectura Técnica — MVVM + Domain Layer (4 capas), Material 3
- ADR-03 (Room/SQLite), ADR-05 (MVVM + Domain), ADR-07 (Navigation Compose), ADR-09 (StateFlow + SharedFlow)
- Modelo de Datos — `plan_assignment`, `exercise_progression`, `exercise_set`, `session_exercise`
- Mapa de Navegación — B1 (Home), E1-E5 (session flow)
- Especificación Visual — Colores semánticos para estado/progresión
- Plan de Entrenamiento — Composiciones por módulo, rangos isométricos
...
**Validado por:** Esteban | **Fecha:** 2026-03-05 | **Enfoque:** Exploratorio
