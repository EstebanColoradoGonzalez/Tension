...
<!-- AUDITORÍA: Completada 2026-02-15 — Verificación exhaustiva archivo-por-archivo contra código implementado (HU-01 a HU-07).

DIMENSIONES AUDITADAS (5):
- A. Código implementado: 7 archivos verificados contra claims del arquitecto. RegisterSetViewModel.kt (146 líneas, CA-08.01/08.04/08.05/08.08 presentes), RegisterSetScreen.kt (307 líneas, 3 variantes E2 confirmadas), RegisterSetUiState.kt (27 líneas, isBodyweight/isIsometric/isWeightEditable), RegisterSetUseCase.kt (3 validaciones: weight≥0, reps≥1, RIR 0-5), SessionRepositoryImpl.kt (268 líneas, lastWeightKg=0.0 para bodyweight/isometric), ExerciseProgressionEntity.kt (status con MASTERED, prescribedLoadKg nullable), strings.xml (7 strings E2 bodyweight/isometric confirmadas).
- B. HU-06 pre-implementación: análisis §6 de HU-06 documenta explícitamente la cobertura de CA-08.01, CA-08.04, CA-08.05, CA-08.08 en E2.
- C. Orden when en WeightField: isIsometric se evalúa antes de isBodyweight (línea 162-163 RegisterSetScreen.kt) — correcto por diseño (isométricos también son bodyweight en el modelo).
- D. ExerciseProgressionEntity: status CHECK incluye 'MASTERED', prescribedLoadKg nullable, sessionsWithoutProgression con default 0 — infraestructura lista para HU-10/HU-11.
- E. Cross-check HUs: HU-03 (flags is_bodyweight/is_isometric en ExerciseEntity), HU-05 (texto de carga diferenciado en E1), HU-06 (pre-implementación E2), HU-07 (sustituto puede ser bodyweight/isométrico sin cambios en E2).

CORRECCIONES APLICADAS (1 LOW):
(L1) RegisterSetViewModel.kt: Arquitecto cita "línea 44" para bodyweight check pero actual es línea 43. Discrepancia off-by-one; sin impacto funcional.

AUDITORÍA PROFUNDA (2026-02-15):
Scope: 7 docs arquitectura + 5 docs business + 32 HUs + 7 archivos de código.
Docs cruzados: Modelo de Datos §3.12/§3.13, Especificación Visual §8 E1/E2/E5, ADR-06, Wireframes E2, Requerimientos RF31/RF32/RF33/RNF12, MDS §6-A Reglas 6-7.
HUs cruzadas (11 con referencias): HU-01 (RNF12 ✅), HU-03 (flags is_bodyweight/is_isometric en ExerciseEntity ✅), HU-04 (CA-04.04 labels diferenciados en D4 ✅ — AGREGADO), HU-05 (texto carga diferenciado E1 ✅), HU-06 (pre-implementación 4 CAs registro, §6 ✅), HU-07 (sustituto bodyweight/isométrico ✅), HU-10 (GAP confirmado: no menciona bodyweight/isometric/MASTERED — correctamente flaggeado en L197 ✅), HU-11 (CA-11.08 excluye bodyweight ✅), HU-13 (E5 Badge Dominado, CA-08.07 trazado en Esp. Visual — AGREGADO a HUs futuras), HU-17 (CA-17.09 prescripciones descarga ✅), HU-19 (CA-19.05 excluye bodyweight ✅), HU-23 (CA-23.06 tendencia reps ✅).
Mapa de Historias: 7 referencias verificadas (RF31/RF32/RF33/RNF12 trazabilidad ✅).
Código verificado: RegisterSetViewModel.kt (146 líneas, 4 CAs ✅), RegisterSetScreen.kt (307 líneas, 3 variantes ✅), RegisterSetUiState.kt (27 líneas, 3 flags ✅), RegisterSetUseCase.kt (20 líneas, 3 require ✅), SessionRepositoryImpl.kt (268 líneas, lastWeightKg=0.0 ✅), ExerciseProgressionEntity.kt (33 líneas, status/prescribedLoadKg/sessionsWithoutProgression ✅), strings.xml (159 líneas, 7 strings E2 bodyweight/isometric ✅).
Observación INFO: CHECK constraints de exercise_progression están en Modelo de Datos doc, no enforced en Room runtime (Room no soporta CHECK annotations). Aplicación valida por código.

CORRECCIONES APLICADAS EN AUDITORÍA PROFUNDA (2 INFO → correcciones):
(I1) HU-04 agregada a "Implementaciones similares analizadas" — CA-04.04 diferencia labels bodyweight/isometric en D4.
(I2) HU-13 agregada a "HUs futuras que dependen de artefactos" — Especificación Visual §8 E5 traza CA-08.07 al Badge 🏆 Dominado (AssistChip M3).
...
Análisis Arquitectónico de HU-08 con hallazgo principal: HU-08 es una historia de naturaleza transversal (cross-cutting) — no requiere trabajo de implementación autónomo. Los 8 CAs se dividen en: 4 CAs de registro (ya implementados en HU-06) y 4 CAs de progresión (diferidos a HU-10/HU-11). El arquitecto verificó exhaustivamente la cobertura CA por CA contra 7 archivos de código. No define hitos de implementación nuevos.

**Nivel de complejidad:**
NULA — HU-08 no introduce componentes nuevos, no modifica archivos existentes, y no requiere tareas de implementación. Es una historia de verificación y documentación de reglas de negocio transversales. Los 4 CAs de registro (CA-08.01, CA-08.04, CA-08.05, CA-08.08) están completamente implementados en el código actual como parte de HU-06. Los 4 CAs de progresión (CA-08.02, CA-08.03, CA-08.06, CA-08.07) serán implementados en HU-10 (motor de clasificación de progresión) y HU-11 (motor de Doble Umbral).

**Riesgos técnicos conocidos:**

Ninguno — no hay trabajo de implementación.
...
- Validación en 2 capas: UI (ViewModel inline) + Domain (UseCase require)
- El campo `exercise_set.reps` almacena tanto repeticiones (estándar/bodyweight) como segundos (isométricos); la interpretación depende de `exercise.is_isometric`

**Dependencias nuevas a instalar:**
Ninguna.

**Estrategia de testing:**
JUnit 4 + MockK + kotlinx-coroutines-test | No se requieren tests nuevos — HU-08 no genera código nuevo | Cobertura: los tests existentes de HU-06 (RegisterSetUseCaseTest) ya cubren los 4 CAs de registro

### Código existente verificado (HU-01 a HU-07 implementados)

| Componente | Archivo | CA cubierto | Estado |
| --- | --- | --- | --- |
| `RegisterSetViewModel` | `ui/session/RegisterSetViewModel.kt` (146 líneas) | CA-08.01, CA-08.04, CA-08.05, CA-08.08 | ✅ Peso=0, isWeightEditable=false, error diferenciado isométrico/estándar, validación inline |
| `RegisterSetScreen` | `ui/session/RegisterSetScreen.kt` (307 líneas) | CA-08.01, CA-08.04, CA-08.05, CA-08.08 | ✅ 3 variantes: estándar, peso corporal, isométrico. WeightField: isIsometric→isBodyweight→else. RepsField: "Segundos sostenidos"/"seg" + referencia 30-45s |
...
- HU-06 (RegisterSet) — Pre-implementó todos los CAs de registro de HU-08 como parte de E2. La decisión fue explícita y documentada en HU-06 análisis §6: "Los CAs de HU-08 referenciados en E2 (CA-08.01, CA-08.04, CA-08.05, CA-08.08) se cubren en este diseño". Esto evitó duplicar test scenarios y reabrir un composable ya validado.
- HU-05 (ActiveSession) — Muestra texto de carga diferenciado en E1: `session_load_bodyweight` ("Peso corporal"), `session_load_isometric` ("Isométrico (30-45s)"), valor en Kg para estándar. Orden del `when` ya correcto: isIsometric → isBodyweight → else.
- HU-04 (AssignExercise) — CA-04.04: diferencia ejercicios con condiciones especiales en D4. Labels "Al fallo técnico" (bodyweight con `isToTechnicalFailure`) y "30-45 seg" (isométricos) en fontStyle italic. Usa flags `isBodyweight`, `isIsometric`, `isToTechnicalFailure` en `PlanExercise` data class.
- HU-03 (CreateExercise) — Originó las flags `is_bodyweight`, `is_isometric`, `is_to_technical_failure` en `ExerciseEntity`. Los 43 ejercicios del seeder tienen flags correctos asignados.
- HU-07 (SubstituteExercise) — Reconoce que un sustituto puede ser bodyweight o isométrico. E2 ya maneja las 3 variantes sin cambios adicionales.

**Patrones de código reutilizados:**

- Validación en 2 capas: (`RegisterSetViewModel` inline + `RegisterSetUseCase` require) → patrón que HU-10/HU-11 deberán seguir para las reglas de progresión
- `exercise_set.reps` como campo dual (repeticiones o segundos) → la interpretación depende de `exercise.is_isometric`, sin columna separada (Modelo de Datos §3.12)
- Orden `when`: isIsometric se evalúa antes de isBodyweight en toda la capa UI (WeightField, ActiveSessionViewModel) porque en el modelo de datos los isométricos son subconjunto de bodyweight (`is_isometric = 1 → is_bodyweight = 1`)

**Mejores prácticas aplicadas:**

- Pre-implementación de variantes cuando el costo marginal es bajo (HU-06 implementó las 3 variantes de E2 en un solo pass)
- Infraestructura de progresión preparada (ExerciseProgressionEntity con status MASTERED, prescribedLoadKg nullable) sin implementar lógica — datos listos para HU-10/HU-11
- Campo `reps` reutilizado para segundos en isométricos, evitando columna adicional con misma semántica (INTEGER ≥ 1, operaciones de suma y comparación idénticas)
...
| --- | --- | --- | --- |
| CA-08.02 | HU-10 (Regla 6 MDS) | `exercise_set.reps` × 4 series por `session_exercise` — datos se persisten correctamente | Lógica de comparación: `SUM(reps)` de 4 series vs sesión anterior. Condición: `exercise.isBodyweight == true` |
| CA-08.03 | HU-11 (Regla 1 MDS) | `exercise.is_bodyweight` flag disponible. `exercise_progression.prescribed_load_kg` nullable | Exclusión: `if (!exercise.isBodyweight) applyDoubleThreshold()`. `Δmin = 0` → no prescribir incremento |
| CA-08.06 | HU-10 (Regla 7 MDS) | `exercise_set.reps` almacena segundos para isométricos. `exercise.is_isometric` disponible | Evaluación: comparar segundos sostenidos (en rango 30-45s) vs sesión anterior del mismo ejercicio |
| CA-08.07 | HU-10 (Regla 7 MDS) | `exercise_progression.status` incluye `'MASTERED'`. CHECK constraint ya definido en schema | Transición: cuando 4 de 4 series ≥ 45s → `status = "MASTERED"`. Estado terminal para isométricos |

**HUs futuras que dependen de artefactos verificados en HU-08:**

- HU-09: Cerrar sesión → trigger que invoca evaluación de progresión (HU-10). Los datos registrados (reps/segundos con peso=0) son la entrada.
- HU-10: Clasificación de progresión → implementará CA-08.02, CA-08.06, CA-08.07. Deberá detectar `isBodyweight`/`isIsometric` y aplicar reglas específicas (Reglas 6 y 7 del MDS §6-A).
- HU-11: Doble Umbral → implementará CA-08.03. CA-11.08 excluye bodyweight (`Δmin = 0`).
- HU-13: Resumen Post-Sesión (E5) → Especificación Visual §8 E5 traza CA-08.07 al "Badge 🏆 Dominado" (AssistChip M3, Tertiary Container, solo isométricos con 4 series ≥ 45s). HU-13 implementará la visualización del estado MASTERED.
- HU-17: Protocolo de descarga → CA-17.09: prescripciones específicas bodyweight (8 reps, RIR 4-5) e isométricos (30 seg, RIR 4-5).
- HU-19: KPIs por ejercicio → CA-19.05: excluye bodyweight de "Velocidad de Progresión de Carga".
- HU-23: Historial de ejercicio → CA-23.06: tendencia por repeticiones totales para bodyweight.

---

## Tareas de Implementación (Developer)

<!-- HU-08 es una historia transversal sin trabajo de implementación autónomo.
     
     Los 4 CAs de registro (CA-08.01, CA-08.04, CA-08.05, CA-08.08) ya están completamente
     implementados en el código actual como parte de HU-06.
     
     Los 4 CAs de progresión (CA-08.02, CA-08.03, CA-08.06, CA-08.07) serán implementados
     en HU-10 y HU-11 cuando el motor de progresión sea desarrollado.
     
     No existen fases de implementación activas. Solo se incluye la Fase N obligatoria. -->
