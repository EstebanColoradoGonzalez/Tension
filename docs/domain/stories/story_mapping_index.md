...
- **Orden:** `HU-39` primero y `HU-40` inmediatamente después —orden crítico—; `HU-41` tercera; `HU-42` paralelizable tras `HU-39`; `HU-43` última, tras `HU-39` y `HU-41`. **`HU-39`, `HU-40`, `HU-41` y `HU-42` están implementadas** (2026-09-07 las dos primeras, 2026-09-11 las otras dos): el esquema es la versión 23, el respaldo la 16, la unidad de comparación del motor es el par (ejercicio, equipamiento), el catálogo tiene 33 zonas jerarquizadas sobre los mismos 14 grupos, el plan por defecto sugiere implemento en sus 35 asignaciones y el panel de Métricas tiene entrada a la vista de 1RM estimado por par (`Flujo O`). **`HU-43` es la única pendiente** y puede arrancar: su prerrequisito —el formulario de ejercicio con equipamiento múltiple y jerarquía de zonas— está entregado.
...
| Historias en Borrador (PO) | 1 (HU-43 — EPIC-10, pendiente de análisis arquitectónico) |
