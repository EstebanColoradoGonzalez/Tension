# HU-16 — Migración a división Pull / Push / Legs
...
| **ID** | HU-16 |
| **ID Legado** | HU-15.5 |
| **Épica** | EPIC-01 — Configuración inicial y estructura del plan |
| **Estado** | Done |
| **Requisitos** | RF04, RF05, RF08 |
| **Dependencias** | HU-01 (BD + seeders), HU-03 (ExerciseSeeder + PlanSeeder + assets), HU-04 (estructura original del plan), HU-15 (métricas — recálculo retroactivo) |
| **Habilita** | HU-17, HU-18, HU-19 (operan sobre la estructura Pull/Push/Legs correcta) |
...
**6. Alertas existentes con `module_code` preservadas — HU-18 (alertas) no estaba implementada en este punto.**

`AlertEntity` tiene columna `module_code` (FK a `module`). Sin embargo: (1) La migración NO modifica `alert` (CA-16.25 correcto). (2) HU-18 (sistema de alertas) aún no estaba implementada al momento de HU-16 — no existían alertas en la BD del ejecutante. (3) Si las hubiera, los valores de FK (`"A"`, `"B"`) siguen siendo claves válidas — solo cambió la descripción del módulo, no su código. No hay acción requerida.

**7. No se necesita `exportSchema = true` ni `MigrationTestHelper` para esta migración.**

El proyecto tiene `exportSchema = false` en `@Database`. Para una migración de datos pura (sin DDL), `MigrationTestHelper` no aporta valor: su propósito es validar cambios estructurales de esquema. Los 4 tests de integridad del Bloque G se implementan como tests instrumentados estándar que ejecutan queries de verificación post-migración sobre una BD real.

**8. Hallazgo en CA-16.26: nombres de columna y tabla incorrectos en la redacción del PO.**

CA original decía que el `profile` almacena la posición actual en la rotación. Verificación contra código real:
...
- Los nombres de columna correctos son: `microcycle_position`, `current_version_module_a`, `current_version_module_b`, `current_version_module_c`, `microcycle_count`.

El **intent** del CA es correcto: el estado de rotación se preserva porque la migración SQL NO toca la tabla `rotation_state`. El cumplimiento es automático.
...
| 5 | `PlanSeeder` | Reemplazar las 66 llamadas `pa()` de módulos A y B con las 66 nuevas según CA-16.15 y CA-16.16. Las 27 de C no cambian | Mayor |
...
| 4 | `Migrations` | Crear `object Migrations` en `data.local.database` con `MIGRATION_6_7` conteniendo los 7 bloques SQL del CA-16.24 | Ninguna |
...
**Nota 4 — Imágenes de ejercicios (assets) vinculadas a `module_code`.**

La función `rememberExerciseBitmap()` en `ExerciseDetailScreen.kt` (HU-03, D2) construye la ruta de assets como `exercises/module-${moduleCode.lowercase()}/$mediaResource.png`. Al cambiar `module_code` de 13 ejercicios, la app buscaría los PNGs en directorios incorrectos. **Mitigación:** Fase 2.5 mueve los 13 archivos PNG a los directorios que corresponden a su nuevo `module_code`.

### Verificación Cruzada de CAs

| CA | Estado | Mecanismo | Implementado en |
|---|---|---|---|
| CA-16.01 | Por implementar | `UPDATE module SET name = '...', group_description = '...' WHERE code = 'A'` | Migration SQL + ModuleSeeder |
| CA-16.02 | Por implementar | `UPDATE module SET name = '...', group_description = '...' WHERE code = 'B'` | Migration SQL + ModuleSeeder |
| CA-16.03 | Automático | C no se toca en migration ni seeders | No requiere acción |
| CA-16.04 | Por implementar | `UPDATE exercise SET module_code = 'B' WHERE id IN (1,2,3,4,5,6,7)` | Migration SQL + ExerciseSeeder |
| CA-16.05 | Por implementar | `UPDATE exercise SET module_code = 'A' WHERE id IN (16,17,18,19,20)` | Migration SQL + ExerciseSeeder |
| CA-16.06 | Por implementar | `UPDATE exercise SET module_code = 'A' WHERE id IN (..., 26)` | Migration SQL + ExerciseSeeder |
| CA-16.07 | Automático | IDs 24,25,27,28,29 ya tienen `module_code = 'B'` | No requiere acción |
| CA-16.08 | Automático | IDs 21,22,23 ya tienen `module_code = 'B'` | No requiere acción |
| CA-16.09 | Automático | IDs 8-15 ya tienen `module_code = 'A'` | No requiere acción |
| CA-16.10 | Automático | IDs 30-43 ya tienen `module_code = 'C'` | No requiere acción |
| CA-16.11 | Por verificar | Test: `SELECT module_code, COUNT(*) FROM exercise GROUP BY module_code` → A=14, B=15, C=14 | Test instrumentado |
| CA-16.12 | Por implementar | `UPDATE exercise_muscle_zone SET muscle_zone_id = 4 WHERE exercise_id = 26` | Migration SQL + ExerciseSeeder |
| CA-16.13 | Automático | KPIs de HU-15 se derivan de `exercise_muscle_zone` vía JOINs — recálculo retroactivo | Sin acción adicional |
| CA-16.14 | Por implementar | `DELETE FROM plan_assignment WHERE module_version_id IN (1,2,3,4,5,6)` | Migration SQL |
| CA-16.15 | Por implementar | 33 INSERTs para A-V1/V2/V3 | Migration SQL + PlanSeeder |
| CA-16.16 | Por implementar | 33 INSERTs para B-V1/V2/V3 | Migration SQL + PlanSeeder |
| CA-16.17 | Automático | IDs 7,8,9 no se tocan | No requiere acción |
| CA-16.18 | Por verificar | Test: conteo por module_version_id → 11×6 + 9×3 = 93 | Test instrumentado |
| CA-16.19 | Por implementar | `ModuleSeeder.kt` actualizado | ModuleSeeder |
| CA-16.20 | Por implementar | `ExerciseSeeder.kt` — 13 ejercicios con module_code actualizado | ExerciseSeeder |
| CA-16.21 | Por implementar | `ExerciseSeeder.kt` — `emz(db, 26, 4)` | ExerciseSeeder |
| CA-16.22 | Por implementar | `PlanSeeder.kt` — 66 nuevas llamadas `pa()` | PlanSeeder |
| CA-16.23 | Por implementar | `TensionDatabase` → `version = 7` + `DatabaseModule` → `.addMigrations(MIGRATION_6_7)` | TensionDatabase + DatabaseModule |
| CA-16.24 | Por implementar | SQL secuencial en `Migrations.MIGRATION_6_7.migrate()` | Migrations |
| CA-16.25 | Automático | Migration SQL no toca `session`, `exercise_set`, `session_exercise`, `alert` | Sin acción requerida |
| CA-16.26 | Automático (con corrección de redacción) | La tabla `rotation_state` no se toca. Columnas reales: `microcycle_position`, `current_version_module_a/b/c`, `microcycle_count` | Sin acción en código |
| CA-16.27 | Por implementar | Test instrumentado: conteo ejercicios por módulo | Test |
| CA-16.28 | Por implementar | Test instrumentado: conteo asignaciones por versión | Test |
| CA-16.29 | Por implementar | Test instrumentado: zona muscular de ejercicio 26 | Test |
| CA-16.30 | Por implementar | Test instrumentado: integridad referencial plan_assignment | Test |
...
**Nivel de complejidad:** BAJA — HU-16 es una operación 100% en la capa de datos. No hay cambios de esquema DDL, no hay componentes de dominio, no hay cambios de UI, no hay nuevas dependencias. El componente nuevo (`Migrations`) es un `object` con una única propiedad `val MIGRATION_6_7`. Los cambios en seeders son actualizaciones de valores literales. El SQL de la migración son 7 bloques de statements planos (UPDATE, DELETE, INSERT) sin lógica condicional. La complejidad real está en la verificación de correctitud: asegurar que los 66 INSERTs de `plan_assignment` coincidan exactamente con el Plan de Entrenamiento actualizado.

**Riesgos técnicos conocidos:**

1. **Inconsistencia Seeder ↔ Migration SQL:** Los seeders y la `MIGRATION_6_7` deben producir exactamente el mismo estado final. Si un INSERT difiere entre ambos, instalaciones nuevas y actualizaciones tendrán catálogos distintos. Mitigación: los 66 INSERTs del SQL están contrastados contra los CAs 16.15/16, el Diccionario de Ejercicios y los seeders. El test instrumentado CA-16.30 (integridad referencial) detectaría inconsistencias.
2. **Sesión `IN_PROGRESS` al momento de la actualización:** Riesgo aceptado (Nota 1). El cierre de sesión evalúa progresión por `exercise_id`, no por `module_code`.
3. **`fallbackToDestructiveMigration()` como red de seguridad:** Si la migración falla, Room destruye la BD y ejecuta los seeders actualizados. Esto preserva la estructura correcta del plan, pero pierde los datos históricos del ejecutante. El riesgo es mitigado porque el SQL fue verificado y los tests instrumentados validan la migración antes del release.
4. **Imágenes de ejercicios (assets) vinculadas a `module_code`:** `rememberExerciseBitmap()` en `ExerciseDetailScreen.kt` (HU-03, D2) construye la ruta de assets como `exercises/module-${moduleCode.lowercase()}/$mediaResource.png`. Al cambiar `module_code` de 13 ejercicios, la app buscaría los PNGs en directorios incorrectos. Mitigación: Fase 2.5 mueve los 13 archivos PNG.
...
- `DatabaseModule`: builder chain: `.addCallback(PrepopulateCallback()).addMigrations(...).fallbackToDestructiveMigration().build()`
...
**Estrategia de testing:** JUnit 4 + AndroidX Test (Room inMemoryDatabaseBuilder) | Tests instrumentados para validación de migración post-apply | Cobertura: 4 tests del Bloque G + 1 test de preservación de tablas históricas.

### Historias Relacionadas Consultadas

- **HU-03:** Creó los seeders (`ModuleSeeder`, `ExerciseSeeder`, `PlanSeeder`), el patrón `PrepopulateCallback → PrepopulateFacade → Seeders`, y las pantallas D1/D2 del Diccionario de Ejercicios — D2 carga imágenes vía `rememberExerciseBitmap()` usando `moduleCode` para construir la ruta de assets. Este es el patrón que se actualiza y la pantalla cuyas imágenes se ven afectadas por la reasignación de `module_code`.
- **HU-04:** Definió la estructura original del plan (`PlanSeeder`) con las versiones A/B/C. Las secciones A y B se recomponen completamente.
- **HU-05:** E1 (sesión activa) y B1 (próxima sesión) muestran `module.name` y listas de ejercicios derivadas de `plan_assignment` — ambos cambian visualmente post-migración. Funcionalidad preservada vía Room Flows reactivos.
- **HU-09:** `rotation_state` no se toca — el ciclo A→B→C sigue igual.
- **HU-10:** `exercise_progression` no se toca — `load_increment_kg` de A y B son ambos 2.5.
- **HU-14:** `deload` no se toca — versiones congeladas siguen referenciando IDs de `module_version` válidos.
- **HU-15:** Implementó métricas que se recalculan automáticamente post-migración porque derivan de `exercise_id` y JOINs a `exercise_muscle_zone`, no de `module_code`.
...
**AC: CA-16.01, CA-16.02, CA-16.03, CA-16.19**

#### Data Layer — Seeder

- [ ] **Modificar `ModuleSeeder`**
  ...
  - [ ] Cambiar línea de Módulo B:
    ...
    ```
  - [ ] Verificar que Módulo C NO se modifica (CA-16.03)
  - [ ] Verificar que `load_increment_kg` permanece 2.5 para A y B, 5.0 para C

### Fase 2: Actualización de ExerciseSeeder

**AC: CA-16.04, CA-16.05, CA-16.06, CA-16.07, CA-16.08, CA-16.09, CA-16.10, CA-16.11, CA-16.20, CA-16.21**

#### Data Layer — Seeder

- [ ] **Modificar `ExerciseSeeder.seedExercises()`**
...
- [ ] **Modificar `ExerciseSeeder.seedExerciseMuscleZones()`**
...
- [ ] **Mover 7 PNGs de Pecho de `module-a/` a `module-b/`** (ejercicios IDs 1-7, ahora `module_code = "B"`)
  - [ ] `assets/exercises/module-a/press_de_banca_maquina.png` → `assets/exercises/module-b/`
  - [ ] `assets/exercises/module-a/press_de_mancuerna_mancuernas.png` → `assets/exercises/module-b/`
  - [ ] `assets/exercises/module-a/press_de_banca_inclinada_maquina.png` → `assets/exercises/module-b/`
  - [ ] `assets/exercises/module-a/flexiones_cuerpo.png` → `assets/exercises/module-b/`
  - [ ] `assets/exercises/module-a/cruce_en_polea_alta_maquina.png` → `assets/exercises/module-b/`
  - [ ] `assets/exercises/module-a/apertura_de_pecho_sentado_maquina.png` → `assets/exercises/module-b/`
  - [ ] `assets/exercises/module-a/apertura_de_pecho_inclinado_maquina.png` → `assets/exercises/module-b/`

- [ ] **Mover 6 PNGs de Bíceps + Elevación hombros de `module-b/` a `module-a/`** (ejercicios IDs 16-20 + 26, ahora `module_code = "A"`)
  - [ ] `assets/exercises/module-b/curl_de_biceps_mancuerna.png` → `assets/exercises/module-a/`
  - [ ] `assets/exercises/module-b/curl_de_biceps_polea.png` → `assets/exercises/module-a/`
  - [ ] `assets/exercises/module-b/curl_de_martillo_cruzado_mancuerna.png` → `assets/exercises/module-a/`
  - [ ] `assets/exercises/module-b/curl_de_martillo_mancuerna.png` → `assets/exercises/module-a/`
  - [ ] `assets/exercises/module-b/curl_de_contraccion_mancuerna.png` → `assets/exercises/module-a/`
  - [ ] `assets/exercises/module-b/elevacion_de_hombros_con_mancuernas_mancuerna.png` → `assets/exercises/module-a/`

- [ ] **Verificar conteo post-reorganización:**
  - [ ] `module-a/`: 14 PNGs (3 Espalda + 5 Abdomen + 5 Bíceps + 1 Elevación hombros)
  - [ ] `module-b/`: 15 PNGs (7 Pecho + 3 Tríceps + 5 Hombro)
  - [ ] `module-c/`: 14 PNGs (sin cambios)

**Nota:** No hay colisiones de nombres entre los archivos de Pecho y los de Bíceps, por lo que no se requiere directorio temporal intermedio.

### Fase 3: Actualización de PlanSeeder

**AC: CA-16.14, CA-16.15, CA-16.16, CA-16.17, CA-16.18, CA-16.22**

#### Data Layer — Seeder

- [ ] **Modificar `PlanSeeder.seedPlanAssignments()`**
  ...
  - [ ] Reemplazar las 33 llamadas `pa()` de **Módulo B** (sección `// ===== Module B =====`) con:
    ...
    ```
  - [ ] Verificar que sección `// ===== Module C =====` NO se modifica (CA-16.17)
  - [ ] **Verificación cruzada:** Contar 33 + 33 + 27 = 93 llamadas `pa()` total (CA-16.18)

#### Verificación cruzada contra Plan de Entrenamiento

- [ ] A-V1: IDs `{10, 8, 9, 16, 18, 17, 19, 11, 12, 13, 14}` — 11 ejercicios
- [ ] A-V2: IDs `{10, 8, 9, 26, 16, 18, 17, 20, 11, 14, 15}` — 11 ejercicios
- [ ] A-V3: IDs `{10, 8, 9, 26, 16, 19, 17, 20, 11, 13, 14}` — 11 ejercicios
- [ ] B-V1: IDs `{1, 3, 6, 4, 27, 24, 25, 28, 21, 22, 23}` — ID 4 usa `FAILURE`
- [ ] B-V2: IDs `{1, 7, 6, 5, 27, 25, 29, 2, 21, 22, 23}` — 11 ejercicios
- [ ] B-V3: IDs `{1, 3, 6, 5, 27, 24, 28, 2, 21, 22, 23}` — 11 ejercicios
...
- [ ] **Crear `Migrations`**
  ...
  - [ ] Implementación completa:
    ...
    ```
  - [ ] **Verificación:** 2 UPDATEs module + 2 UPDATEs exercise + 1 UPDATE exercise_muscle_zone + 1 DELETE + 66 INSERTs = 72 execSQL calls
...
- [ ] **Modificar `DatabaseModule`**
  ...
  - [ ] Insertar `.addMigrations(Migrations.MIGRATION_6_7)` ANTES de `.fallbackToDestructiveMigration()`:
    ```kotlin
    // ANTES:
    return Room.databaseBuilder(context, TensionDatabase::class.java, "tension_database")
        .addCallback(PrepopulateCallback())
        .fallbackToDestructiveMigration()
        .build()
...
**AC: CA-16.25, CA-16.26, CA-16.27, CA-16.28, CA-16.29, CA-16.30**

#### Tests instrumentados

- [ ] **Crear `MigrationV6ToV7Test`**
  ...
  - [ ] Setup: BD en memoria vía `Room.inMemoryDatabaseBuilder()` con version 7 y `addMigrations(Migrations.MIGRATION_6_7)`. Los tests validan el estado final correcto (converge por migration o por seeder).

  - [ ] **Test 1 — Conteo de ejercicios por módulo** (CA-16.27):
    ```kotlin
    @Test
  ...
  - [ ] **Test 2 — Conteo de asignaciones por versión** (CA-16.28):
    ```kotlin
    @Test
  ...
  - [ ] **Test 3 — Zona muscular de Elevación de hombros** (CA-16.29):
    ```kotlin
    @Test
  ...
  - [ ] **Test 4 — Integridad referencial de plan_assignment** (CA-16.30):
    ```kotlin
    @Test
  ...
  - [ ] **Test 5 — Datos históricos no tocados** (CA-16.25, CA-16.26):
    ```kotlin
    @Test
...
- [ ] **Escenario 1:** Instalación limpia (desinstalar app → instalar) — verificar que la pantalla de inicio muestra los módulos correctos y el plan contiene los ejercicios nuevos
...
### Verificación cruzada de CAs — Resumen

| CA | Fase |
|---|---|
| CA-16.01–02 | Fase 1 (ModuleSeeder) + Fase 4 (Migration SQL UPDATEs module) |
| CA-16.03 | Automático — C no se toca en ninguna fase |
| CA-16.04–06 | Fase 2 (ExerciseSeeder) + Fase 2.5 (mover PNGs) + Fase 4 (Migration SQL UPDATEs exercise) |
| CA-16.07–10 | Automáticos — ejercicios ya tienen module_code correcto |
| CA-16.11 | Fase 6 (Test 1) |
| CA-16.12 | Fase 2 (ExerciseSeeder emz) + Fase 4 (Migration SQL UPDATE exercise_muscle_zone) |
| CA-16.13 | Automático — KPIs de HU-15 se recalculan vía JOINs a exercise_muscle_zone |
| CA-16.14 | Fase 4 (Migration SQL DELETE plan_assignment) |
| CA-16.15–16 | Fase 3 (PlanSeeder) + Fase 4 (Migration SQL 66 INSERTs) |
| CA-16.17 | Automático — módulo C no se toca |
| CA-16.18 | Fase 6 (Test 2) |
| CA-16.19 | Fase 1 (ModuleSeeder) |
| CA-16.20–21 | Fase 2 (ExerciseSeeder) |
| CA-16.22 | Fase 3 (PlanSeeder) |
| CA-16.23 | Fase 5 (TensionDatabase version + DatabaseModule addMigrations) |
| CA-16.24 | Fase 4 (Migrations SQL completo) |
| CA-16.25–26 | Automáticos — migration SQL no toca tablas históricas + Fase 6 (Test 5) |
| CA-16.27 | Fase 6 (Test 1) |
| CA-16.28 | Fase 6 (Test 2) |
| CA-16.29 | Fase 6 (Test 3) |
| CA-16.30 | Fase 6 (Test 4) |
