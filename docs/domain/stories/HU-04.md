# Historia de Usuario: `HU-04` — Consultar y gestionar Plan de Entrenamiento
...
- **Requisitos cubiertos:** RF05, RF06, RF08, RF63, RF64
...
- **Entidades Afectadas:** `ModuleVersionEntity`, `PlanAssignmentEntity`, `ModuleEntity`, `ExerciseEntity` (lecturas y escritura via insert/delete en `plan_assignment`) (Referencia a `docs/architecture/domain_and_state_model.md`).
- **Mutaciones de Estado:** Insert en `plan_assignment` (CA-04.07), Delete de `plan_assignment` (CA-04.08). No se agregan tablas nuevas — las entidades ya existen desde HU-03 seed data.

### 4.2. Contrato de Interfaces (Triggers / API)

- **Trigger / Endpoint consumido:** `Tab Plan — D3 TrainingPlanScreen` | `D3 ListItem click → D4 PlanVersionDetailScreen` | `D4 FAB → Bottom Sheet asignación` | `D4 IconButton Delete → AlertDialog desasignación` (Referencia a `docs/architecture/interfaces_contract.md`).
- **Payload requerido:** `moduleVersionId: Long` (argumento de navegación D3→D4). Para asignación: `exerciseId: Long`, `sets: Int`, `reps: String`.

### 4.3. UI / Assets

- **Componente Visual:** `D3 — TrainingPlanScreen` (reemplazo del stub — 3 módulos × 3 versiones con conteos), `D4 — PlanVersionDetailScreen` (lista de ejercicios con prescripción + asignación/desasignación via FAB + AlertDialog).
