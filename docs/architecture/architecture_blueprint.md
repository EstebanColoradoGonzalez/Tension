...
- **WebView del Sistema (Android System WebView / Chrome):** Componente de la plataforma que renderiza el árbol de entrenamiento en 3D (HU-38). Es la **única** dependencia externa **opcional por diseño**: cuando falta, está deshabilitada o su versión es anterior a la mínima soportada, la pantalla del árbol presenta la representación nativa de HU-37 y no pierde ninguna función. Se usa exclusivamente para cargar un asset local empaquetado (`assets/tree/tree.html`) y **no navega a contenido remoto**: la aplicación no declara el permiso `INTERNET`, de modo que la restricción no depende de la configuración del WebView sino de que el sistema operativo no lo permita (ver ADR-021).
...
- **Excepción declarada a la exclusión de red (HU-38):** La incorporación de un WebView y de Three.js **no abre ninguna puerta a la red**. La librería y el HTML viajan **dentro del APK** como assets versionados en Git; no hay CDN, ni descarga en tiempo de ejecución, ni permiso `INTERNET`. La app sigue operando 100 % offline (RNF-09) y la exclusión de backend remoto se mantiene intacta. Lo que la excepción documenta es la **entrada de una tecnología de renderizado ajena a Compose**, acotada a una sola pantalla (ver ADR-021).
...
- **Contenedor `UI-01`: Capa de Presentación (UI Layer)**
  ...
  - **Dependencias clave:** Jetpack Compose BOM, Compose Material 3, Material Symbols / Icons Extended, Navigation Compose, Lifecycle Runtime Compose (`collectAsStateWithLifecycle`). **Desde HU-38** se añade el `WebView` de la plataforma (`android.webkit`, sin `androidx.webkit`) y **Three.js 0.160.0 empaquetado como asset** — la única dependencia no-Jetpack del proyecto y la única JavaScript. Vive en una sola pantalla y **no añade ninguna dependencia Gradle**: el build UMD de Three.js se carga con un `<script>` clásico desde `file:///android_asset`.
...
- **Contenedor `DB-01`: Base de Datos Local (SQLite via Room)**
  - **Naturaleza Técnica:** SQLite nativo de Android, accedido exclusivamente a través de Room ORM. Versión de esquema actual: **24** (migraciones registradas v1→v2…v18→v19; **ni v20, ni v21, ni v22, ni v23, ni v24 tienen migración** por ADR-019). La frontera está declarada en el código como `Migrations.LAST_MIGRATED_VERSION` (**19**): por debajo de ella la cadena es continua y sin huecos, y el salto de ahí al esquema vigente es la excepción, que sube historia por historia de forma deliberada. Archivo único en el directorio interno de la app: `filesDir/databases/tension.db`. Sin cifrado (ADR-15).
...
- **De `UI-01` al render web (HU-38):** El propósito es entregar al código JavaScript el estado que debe dibujar. Utiliza `WebView.evaluateJavascript` invocando `window.tensionTree.setState(healthScore, stageCode)` — **exactamente dos parámetros**: salud y etapa. La calidad de render y el tema no viajan por aquí; entran como *query string* al cargar el asset, porque son propiedades del dispositivo y del sistema, no del estado del árbol.
- **Del render web a `UI-01` (HU-38):** El propósito es reportar si el render está disponible. Utiliza un objeto `@JavascriptInterface` (`TreeBridge`) con `onReady()` y `onFailure(reason)`. **No transporta ningún dato de dominio**: son señales sobre el estado del render, y su único efecto es elegir entre el modelo 3D y el ícono nativo. Se invoca desde un hilo del WebView y se reenvía al hilo principal.

> **Nota:** No existen flujos de red externos. Toda comunicación es local entre contenedores del mismo proceso Android. El protocolo es Kotlin Coroutines + Flow para comunicación reactiva asíncrona entre capas. El canal nativo ↔ web de HU-38 tampoco es red: es comunicación entre dos motores dentro del mismo proceso, sobre un asset local.
...
- **Módulo / Componente:** `NavHost + Routes` (ui.navigation)
  - **Responsabilidad (SRP):** Definir el grafo de navegación completo (27 destinos en 10 nested graphs — el último, `one-rm`, dentro del grafo de Métricas), gestionar el back stack, mapear rutas a Screens y gestionar la visibilidad condicional del Bottom Navigation Bar.
...
- **Módulo / Componente:** Componentes reutilizables (ui.components)
  ...
  - **Nota (HU-38):** `TreeIcon` **no se elimina ni se modifica**: pasa a ser el **fallback permanente** del render 3D. La elección entre representaciones vive en un único sitio, `TreeVisual`, y el ícono está compuesto desde el primer instante — el WebView se superpone con opacidad cero y aparece con un fundido cuando reporta su primer fotograma, de modo que el área nunca queda vacía ni parpadea. Un fallo, temprano o tardío, se ve como un árbol que no cambió de forma; **no hay mensaje de error** porque no hay nada que el ejecutante deba hacer.

- **Módulo / Componente:** Render 3D del árbol (ui.tree, HU-38)
  - **Responsabilidad (SRP):** Presentar el árbol como modelo tridimensional generado por código, rotable con el dedo y con transiciones continuas entre estados de salud, dentro del área que HU-37 reservó. **Es el único WebView de la aplicación** y vive exclusivamente en la pantalla del árbol.
  - **Interfaces Expuestas:** `Tree3DView(healthScore, stage, isDarkTheme, onReady, onFailure, modifier)` y `isTree3DSupported()`. El composable no decide si se ve: intenta renderizar y reporta el resultado; quien elige es la pantalla.
  - **Dependencias Internas:** `TreeGrowthStage` (dominio, solo lectura del enum), `TreeBridge`, `TreeRenderQuality`, `TreeWebViewSupport` y los tres assets de `assets/tree/`. **Ninguna dependencia hacia `DOM-01` más allá del enum, ni hacia `DAT-01`.**
  - **Estructuras Atómicas Clave:** `Tree3DView.kt` (integración nativa, ciclo de vida y detección de fallo), `TreeBridge.kt` (puente `@JavascriptInterface`, con `post` inyectado para ser verificable en JVM), `TreeRenderQuality.kt` (predicción pura de calidad en tres niveles), `TreeWebViewSupport.kt` (versión mínima de WebView), `assets/tree/tree.html` (shell sin texto), `assets/tree/tree.js` (generación procedural, cámara acotada y sonda de rendimiento), `assets/tree/three.min.js` (Three.js 0.160.0, build UMD).
  - **Fronteras verificables:** el WebView no navega fuera de su asset (`shouldOverrideUrlLoading` → `true` siempre, y la app sin permiso `INTERNET`); su ciclo de vida se cierra en `onRelease` desregistrando el puente **antes** de destruir la vista; el árbol se genera por código sin ningún `.glb` ni `.gltf`; y el presupuesto de rendimiento manda sobre la fidelidad — la sonda baja un escalón de calidad si la medida real no lo cumple.

  - **Nota (HU-42):** `MetricNavigationEntry` es la única forma de los tres accesos de `Flujo G` —1RM, volumen y tendencia—, y vive en `MetricCard.kt` junto al resto de la anatomía de presentación de ese flujo por la misma razón que el resto: que el orden de los elementos sea una propiedad del sistema y no una convención repetida tres veces. Antes de esta historia los dos accesos existentes eran `TextButton` con una flecha dentro de la cadena; unificarlos es un cambio de presentación sin lógica, y deja los tres hermanos incapaces de divergir.
...
- **Módulo / Componente:** Motor de Reglas (domain.rules)
  ...
  - **Nota (HU-39):** `ExternalLoadRule.isCaptureEnabled(isBodyweight, isIsometric, equipmentName)` decide si una serie captura carga externa, y lo decide el **implemento** y no solo la marca del ejercicio: `Peso Corporal` registra 0 en cualquier ejercicio, `Peso Añadido` es lo único que habilita la captura sobre un ejercicio de peso corporal —y `requiresPositiveLoad` exige que sea mayor que 0—, y sobre un ejercicio de peso corporal `Barra Fija` y `Máquina` registran 0, porque la carga de una máquina asistida es un contrapeso que **resta** esfuerzo. Decidir solo por la marca dejaría a `Crunch Abdominal`, `Sentadilla Búlgara` y `Zancadas (Lunges)` —que admiten `Peso Corporal` sin estar marcados como tal— pidiendo un peso para una serie hecha con el propio cuerpo. Compara contra el nombre del catálogo, que es dato semilla en español, igual que `LoadIncrementResolver` compara contra el grupo muscular. **No forma parte del motor de decisión:** gobierna la captura, no la progresión.
  - **Nota (HU-40):** `ProgressionConsolidationRule` deriva la lectura del ejercicio a partir de sus pares `(ejercicio, equipamiento)`, que desde esta historia son la unidad de comparación de todo lo que compara pesos entre sesiones. `consolidate(pairClassifications)` realiza la **disyunción** de CA-40.04 —conserva el mejor par clasificado con el orden `POSITIVE_PROGRESSION > MAINTENANCE > REGRESSION > null`, de modo que el ejercicio progresa si alguno de sus pares progresó y `null` solo sobrevive cuando ningún par tiene historial— e `isInPlateau(pairCounters, effectiveThreshold)` la **conjunción** de CA-40.05, que exige que todos los pares hayan alcanzado el umbral y responde `false` para un ejercicio sin pares, porque una conjunción vacía sería cierta por vacuidad. Ninguna de las dos persiste nada: la consolidación se deriva en cada evaluación, así que no hay copia que pueda desincronizarse de los pares. Las siete reglas del motor —`DoubleThresholdRule`, `ProgressionClassificationRule`, `PlateauThresholdRule`, `DeloadLoadRule`, `DeloadNeedRule`, `ProgressionRateRule` y `PrefilledLoadRule`— **no cambiaron**: cambia con qué datos se las invoca, no lo que calculan.
  - **Nota (HU-43):** `SessionAdjustmentRule` decide qué composición admite la sesión de hoy: la prescripción por defecto del ejercicio añadido —3 series, y el rango ajustado al modo del ejercicio con el mismo vocabulario que el plan—, el presupuesto de retiro y los dos veredictos de retirada. **No forma parte del motor de decisión**, igual que `OneRmRule` y `ExternalLoadRule`: vive aquí porque es Kotlin puro y verificable en JVM, no porque prescriba nada. Devuelve un `WithdrawalVerdict` y no un booleano porque la interfaz muestra **la** causa y no *una* causa, y el **orden de evaluación queda fijado en la regla** —series antes que presupuesto—: la serie registrada es la única causa que no se levanta añadiendo nada. Los dos veredictos son distintos porque la aritmética lo es: retirar del plan exige presupuesto (`añadidos − retirados >= 1`), deshacer un añadido exige que la invariante siga cierta después (`añadidos − 1 >= retirados`). La regla **no** conoce el nombre del ejercicio a reponer; lo resuelve quien la llama, y siempre existe alguno cuando el veredicto es negativo — el KDoc lo demuestra.
  - **Nota (HU-42):** `OneRmRule` estima el 1RM de una serie y decide si la serie tiene derecho a producirlo. **No forma parte del motor de decisión**, igual que `ExternalLoadRule`: vive aquí porque es Kotlin puro y verificable en JVM, no porque nada prescriba carga con ella. `qualifies(reps, rir, hasExternalLoad, weightKg)` reúne las cuatro condiciones de una vez —exactamente 10 repeticiones, RIR exactamente 1, carga externa y peso mayor que 0—, y el tercer término es el veredicto de `ExternalLoadRule`, de modo que las exclusiones de CA-42.05 **no se reimplementan**: se invocan. `estimate(weightKg, reps)` escribe la fórmula general `Peso / [1.0278 − (0.0278 × Reps)]` y no la constante `× 1.3337` que resulta a 10 repeticiones, porque la constante escondería que el denominador se anula en 36.97 repeticiones; la equivalencia se afirma en el test, que es donde vale algo. La monotonía **no está en la regla**: es una comparación contra el valor guardado, y vive donde la transacción ya existe.
...
- **Módulo / Componente:** Use Cases (domain.usecase)
  ...
  - **Interfaces Expuestas:** `operator fun invoke(params)`: función de invocación única por Use Case. Ejemplos: `StartSessionUseCase`, `RegisterSetUseCase`, `CloseSessionUseCase`, `DetectPlateauUseCase`, `ActivateDeloadUseCase`, `GetExerciseHistoryUseCase`, `ExportBackupUseCase`. HU-36 añade `GetTodaySessionUseCase` (reemplaza `GetNextSessionInfoUseCase`), `GetReassignableRoutinesUseCase`, `SetTemporaryRoutineUseCase`, `ClearTemporaryRoutineUseCase`, `GetWeekDayPlanUseCase` y `UpdateRoutineWeekDaysUseCase` — este último es la edición **permanente** de la relación día → rutina, distinta de la reasignación temporal de una sesión. `SkipTodayUseCase` y `UndoSkipTodayUseCase` cancelan el día y lo revierten; el primero descarta de paso una sesión en curso sin series, y se rechaza en cuanto hay una registrada. `ResolveStaleSessionUseCase` resuelve al arrancar —y al cruzar la medianoche— la sesión que quedó abierta de un día anterior: la cierra si tiene series, la descarta si no. HU-39 añade `AddExerciseEquipmentUseCase`, `RemoveExerciseEquipmentUseCase`, `GetExerciseEquipmentIdsUseCase` y `GetExerciseEquipmentWithSetsUseCase` — la retirada devuelve el motivo del rechazo en lugar de lanzarlo, porque la pantalla lo presenta como error del campo junto a la casilla que el ejecutante intentó desmarcar. HU-42 añade `GetOneRmListUseCase`, que agrupa la lista plana de pares en una tarjeta por ejercicio y es **el único caso de uso del 1RM**: no hay ninguno de escritura, porque el récord se persiste desde la transacción que registra la serie. HU-43 añade `AddExerciseToSessionUseCase`, `WithdrawFromSessionUseCase`, `GetAddableExercisesUseCase`, `GetSessionAdjustmentUseCase` y `CreateAndAddExerciseToSessionUseCase` — este último **compone** los dos primeros sin transacción conjunta, porque CA-43.02 exige que el ejercicio creado permanezca en el Diccionario aunque el añadido falle, y envolverlos produciría justo el resultado contrario. Ninguno de los cinco conoce `PlanRepository`: que el ajuste sea temporal se sostiene por construcción, no por disciplina. HU-37 añade `GetTreeStateUseCase` y `RecalculateTreeStateUseCase`; el segundo es invocado desde `CloseSessionUseCase`, desde `ImportBackupUseCase` y desde `MainViewModel` **inmediatamente después** de `ResolveStaleSessionUseCase`, nunca antes — el orden es la garantía de que el árbol no se marchite para quien sí entrenó.
...
- **Módulo / Componente:** Interfaces de Repository (domain.repository)
  ...
  - **Interfaces Expuestas:** Interfaces Kotlin: `SessionRepository`, `ExerciseRepository`, `PlanRepository`, `ProfileRepository`, `ProgressionRepository`, `AlertRepository`, `DeloadRepository`, `RotationRepository`, `BackupRepository`, `WeekDayRepository`, `TreeRepository`, `OneRmRepository`.
...
- **Módulo / Componente:** Repository Implementations (data.repository)
  ...
  - **Nota (HU-42):** `OneRmRepositoryImpl` es **solo de lectura**, y su contrato también: expone un único `getAll()` y ningún método de escritura. El récord lo escribe `SessionRepositoryImpl.registerSet` dentro de la transacción que inserta la serie —junto al `insertIfNotExists` de `exercise_progression` que ese método ya hacía—, y no en un paso best-effort posterior al modo del árbol. La divergencia es deliberada: el árbol es **derivable** y un recálculo fallido lo repara el siguiente, mientras que el 1RM es un **récord** que no se reconstruye, así que una escritura perdida sería un máximo perdido para siempre. Lo que sí se conserva del modelo de HU-37 es la dirección de la dependencia, que es lo que importa: nada del sistema **lee** el 1RM.
...
- **Módulo / Componente:** Entities de Room (data.local.entity)
  ...
  - **Interfaces Expuestas:** 24 data classes con anotaciones Room: `@Entity`, `@PrimaryKey`, `@ForeignKey`, `@Index`, `@ColumnInfo`.
  - **Estructuras Atómicas Clave:** `RoutineEntity`, `MuscleZoneEntity`, `EquipmentTypeEntity`, `ExerciseEntity`, `ExerciseMuscleZoneEntity`, `ExerciseEquipmentEntity`, `RoutineVersionEntity`, `PlanAssignmentEntity`, `ProfileEntity`, `WeightRecordEntity`, `SessionEntity`, `SessionExerciseEntity`, `ExerciseSetEntity`, `ExerciseProgressionEntity`, `RotationStateEntity`, `RoutineCurrentVersionEntity`, `WeekDayEntity`, `DailyRoutineOverrideEntity`, `DaySkipEntity`, `TreeStateEntity`, `DeloadEntity`, `DeloadFrozenVersionEntity`, `AlertEntity`, `ExerciseOneRmEntity`, `SessionWithdrawalEntity`.
...
- **Módulo / Componente:** Seed Data (data.local.seed)
  ...
  - **Dependencias Internas:** `ExerciseSeeder` (38 ejercicios base, **87 relaciones ejercicio-zona con jerarquía** —52 principales y 35 secundarias— y **100 relaciones ejercicio-equipamiento**), `BaseDataSeeder` (**33 zonas musculares** y **15 tipos de equipamiento atómicos**), `PlanSeeder` (6 rutinas, 6 versiones y 35 asignaciones del plan predeterminado, **cada una con su equipamiento sugerido**), `WeekDaySeeder` (7 días y su relación con la rutina que les corresponde). `WeekDaySeeder` se ejecuta **después** de `PlanSeeder`: la clave foránea a `routine` exige que las rutinas existan.
  - **Estructuras Atómicas Clave:** Los datos residen en estructuras Kotlin puras — `ExerciseCatalog` (`List<SeedExercise>`), `EquipmentCatalog` (`List<SeedEquipmentType>`), `MuscleZoneCatalog` (`List<SeedMuscleZone>`), `DefaultPlan` (`List<SeedRoutine>` + `List<SeedAssignment>`) y `DefaultWeekDays` (`List<SeedWeekDay>`) — y cada Seeder se limita a mapearlas a `ContentValues`. Sin dependencia de Android, los datos semilla son verificables por tests JVM.
  - **Nota (HU-39):** `EquipmentCatalog` se extrajo de `BaseDataSeeder` por la misma razón por la que `ExerciseCatalog` existe: las tres tablas cerradas de la historia —los 15 tipos, los 37 ejercicios con sus nombres y las 97 relaciones— se afirman en tests JVM sin emulador. El `id` de cada tipo **encoda su posición declarada** en el catálogo, que es el orden en que la lista de casillas del formulario los presenta; por eso `EquipmentTypeDao` ordena por `id` y no por nombre.
  - **Nota (HU-41):** `MuscleZoneCatalog` se extrajo por el mismo motivo, y resuelve el orden de otra forma. El `id` **no puede** encodar la posición declarada: HU-29 fijó que un renombrado conserva su identificador, así que el `id` encoda la historia del catálogo —17 zonas conservan el suyo, 16 nuevas ocupan 21-36 y los ids 4, 7 y 19 quedan libres sin reutilizarse—. El orden que el selector necesita es anatómico y vive en `muscle_zone.sort_order`; por eso `MuscleZoneDao` ordena por esa columna y no por `id` ni por nombre.
...
- **`RF-04, RF-07, RF-61, RF-62` (Diccionario de Ejercicios):** Catálogo base en `ExerciseSeeder` (seed). Creación personalizada en `CreateExerciseUseCase`. Filtros en `GetExercisesFilteredUseCase`. Presentación en `ExerciseDictionaryScreen` / `ExerciseDetailScreen` con `CatalogViewModel`. **Desde HU-39** el equipamiento es una relación N:M (`exercise_equipment`): el ejercicio declara los implementos que admite y la serie declara cuál se usó. Selección múltiple en `EquipmentMultiSelector`, edición inmediata desde la ficha (`AddExerciseEquipmentUseCase` / `RemoveExerciseEquipmentUseCase`) y filtro que coincide si **alguna** opción coincide.
...
- **`HU-38` (El árbol en 3D) — continuación de la excepción de alcance, sin ampliarla:** Sustituye **únicamente** la representación visual del árbol dentro de `TreeScreen`: `TreeVisual` elige entre `Tree3DView` (WebView + Three.js empaquetado) y `TreeIcon` (fallback nativo permanente). Predicción de calidad en `TreeRenderQuality`, soporte de WebView en `TreeWebViewSupport`, puente en `TreeBridge`. **No toca nada de `DOM-01`, `DAT-01`, `DB-01` ni `DI-01`**: ni entidad, ni consulta, ni use case, ni versión de esquema, ni formato de respaldo, ni ruta de navegación. La tarjeta de Inicio sigue siendo nativa de forma permanente (RNF-01). **No mapea ningún RF** y la exclusión de `system_definition_document.md` §2.1 se mantiene intacta (ver ADR-020 y ADR-021).
- **`HU-43` (ajuste temporal de la sesión del día) — cierre de la partición de HU-39:** Regla pura `SessionAdjustmentRule` en `DOM-01`. Marca `session_exercise.is_extra` y prescripción propia (`prescribed_sets`, `prescribed_reps`), más la tabla `session_withdrawal` en `DB-01`. Escritura por `SessionRepositoryImpl.addExerciseToSession` y `withdrawFromSession`, cada una en su transacción; lectura por `getAddableExercises` y `getSessionAdjustment`. Presentación en `ActiveSessionScreen` con `AddExerciseSheet` y el menú contextual de cada fila (`E1-T2`, `E1-T3`), reutilizando `CreateExerciseScreen` con un argumento de ruta opcional. **La frontera de la historia es que el plan no se toca**, y es verificable: ningún camino nuevo alcanza `PlanRepository` ni escribe en `plan_assignment`. La segunda frontera es simétrica y va en la dirección contraria — **ninguna consulta de agregación filtra por `is_extra`**: las series del añadido cuentan para progresión, tonelaje, KPIs y 1RM como cualquier otra (CA-43.08). Reintroduce la capacidad que `HU-34` retiró, y lo hace **sin** reintroducir su defecto: el ejercicio añadido es un `session_exercise` más, sin ejercicio de origen y por tanto sin segunda identidad dentro de la sesión.
- **`HU-42` (1RM estimado por par) — tercera funcionalidad aislada, con una diferencia declarada:** Regla pura `OneRmRule` en `DOM-01`. Récord persistido en `exercise_one_rm` y **escrito desde `SessionRepositoryImpl.registerSet`**, dentro de la transacción que inserta la serie. Lectura por `OneRmRepositoryImpl` → `ExerciseOneRmDao` y `GetOneRmListUseCase`. Presentación en `OneRmScreen` con `OneRmViewModel` (`Flujo O`), alcanzada desde la entrada `G1-T4` del panel de Métricas. **No mapea ningún RF**: es una lectura de fuerza máxima que no alimenta ninguna decisión — ni la prescripción de carga, ni el Doble Umbral, ni la meseta, ni la descarga, ni las alertas, ni los KPIs leen esta tabla. A diferencia de `HU-37` y `HU-38`, esta historia **sí toca `DAT-01` y `DB-01`** —una tabla, un DAO, una escritura dentro de `registerSet` y el formato de respaldo—, y la frontera que conserva intacta es la de la lectura: la dependencia sigue siendo unidireccional (ver ADR-020 para el modelo, y el refinamiento de la historia para por qué la escritura es transaccional y no best-effort).
...
- **Decisión:** `TensionDatabase.version` sube sin migración y **sin** `fallbackToDestructiveMigration()`. Aplicado a v17 (`week_day`, `daily_routine_override`), a v18 (`day_skip`), a v19 (`tree_state`, HU-37), a **v20** (`exercise_equipment`, `exercise_set.equipment_type_id` y la retirada de `exercise.equipment_type_id`, HU-39), a **v21** (`exercise_progression` reclaveada por `(exercise_id, equipment_type_id)` y la nueva `session_exercise_progression`, HU-40) a **v22** (`muscle_zone.sort_order`, `exercise_muscle_zone.is_primary` y `plan_assignment.suggested_equipment_type_id`, HU-41 — tres columnas `NOT NULL` sin default, ninguna tabla nueva) a **v23** (`exercise_one_rm`, HU-42 — una tabla nueva, ninguna columna añadida a las existentes) y a **v24** (`session_exercise.is_extra`, `prescribed_sets` y `prescribed_reps`, más la nueva `session_withdrawal`, HU-43 — tres columnas y una tabla). Las migraciones históricas se conservan intactas: describen el esquema de su época. Las de `16→17`, `17→18` y `18→19`, ausentes durante tres versiones, se repusieron después de dejar la aplicación incapaz de abrir cualquier base existente — y de ahí sale la forma que toma la guarda.
- **La guarda de continuidad es explícita, no implícita.** `Migrations.LAST_MIGRATED_VERSION` declara hasta dónde llega la cadena. `MigrationV16ToV19Test.la_cadena_de_migraciones_no_tiene_huecos` afirma dos cosas: que la cadena no tiene huecos y que su última migración termina exactamente en esa constante. Lo que **no** afirma es que llegue a la versión del esquema, porque esta ADR lo hace falso por decisión de producto. Con ello el hueco deja de ser silencioso: subir el esquema sin migración obliga a mover una constante con nombre y KDoc, revisable en el diff, en lugar de simplemente olvidar un archivo. Una segunda prueba impide que el esquema quede por detrás de la última migración.
- **Formato de respaldo:** hoy en `SCHEMA_VERSION = 17` (v14 con HU-40, v15 con HU-41, v16 con HU-42, **v17 con HU-43**). El recorrido hasta el 13 fue: v10 incorporó `week_day` y `daily_routine_override` —omitirlas perdería la relación día → rutina en cada restauración—, v11 añadió `day_skip`, v12 añadió `tree_state` y **v13 añade `exercise_equipment` y el `equipment_type_id` de cada serie**. v12 aceptaba también el formato inmediatamente previo porque el árbol es derivable del historial y restaurar sin él nunca dejaba un estado inválido. **v13 no acepta ninguno anterior:** el equipamiento de la serie no se deriva de nada — un respaldo anterior no dice con qué implemento se hizo cada serie, y la columna es `NOT NULL` — así que aceptarlo exigiría inventar un valor por serie, que es la importación parcial que HU-39 prohíbe. Los caminos de importación de v8 y v11/v12 se retiraron del código.
- **Consecuencias:** Excepción documentada a RNF19, limitada a la fase beta. Una base anterior falla al abrir con la excepción de Room, que es la señal de reinstalar; la app nunca borra datos por su cuenta. Con HU-39 el historial de sesiones anterior **se pierde**, y queda declarado como consecuencia aceptada. Los respaldos en formatos 8 a 12 dejan de poder restaurarse, y con HU-40 tampoco el 13: su `exercise_progression` no dice con qué implemento se acumuló cada estado, y `equipment_type_id` es ahora `NOT NULL` y parte de la clave primaria. Con HU-41 tampoco el 14: le faltan la jerarquía de `exercise_muscle_zone` y el implemento sugerido de `plan_assignment`, que son decisiones y no cálculos, y CA-41.10 exige reproducir el catálogo y el plan **sin recalcularlos**. Con HU-42 tampoco el 15: no lleva `exercise_one_rm`, y el 1RM es un récord acumulado que recalcular desde las series podría perder — CA-42.08 exige restaurarlo **sin recalcularlo**. Con HU-43 tampoco el 16, y por partida doble: no lleva `session_withdrawal`, y `validateBackup` rechaza por incompleto cualquier respaldo al que le falte una tabla del orden de inserción, así que en cuanto la tabla entra ningún v16 pasa la validación; el motivo de fondo se sostiene solo, porque **el ajuste de la sesión no se deriva del plan** y un v16 no dice qué ejercicio se añadió ni cuál se retiró. Ningún test ejerce el arranque en v20, v21, v22, v23 ni v24 porque no hay migración que probar; lo que se verifica es la semilla sobre instalación fresca (`CatalogoSembradoTest`, `MuscleZoneCatalogTest`, `ExerciseCatalogTest`, `DefaultPlanTest` y validación manual). **La guarda de continuidad no se mueve con estas subidas**: `LAST_MIGRATED_VERSION` sigue en 19 y `MigrationV16ToV19Test` sigue pasando sin tocarse, que es precisamente lo que demuestra que funciona — subir el esquema sin migración es una decisión que el diff enseña. **El criterio lo heredaron las cuatro historias hermanas de HU-39** (`HU-40` … `HU-43`): cada una extendió el esquema y se resolvió del mismo modo. Con `HU-43` la partición queda cerrada, y con ella cinco formatos de respaldo invalidados consecutivamente — el 12, el 13, el 14, el 15 y el 16. Queda planteado, fuera del alcance de estas historias, si el formato debe volver a ser compatible hacia atrás o si la beta asume que un respaldo solo vale dentro de una misma versión.
...
### ADR-021: El árbol 3D por generación procedural en WebView, sin assets externos

- **Contexto:** HU-38 pide un árbol tridimensional rotable con el dedo, con transiciones continuas entre estados de salud, dentro de una aplicación que hasta ese momento es **Compose puro, sin ninguna dependencia de terceros más allá del stack de Jetpack, 100 % offline (RNF-09) y distribuida como APK firmado donde cada byte cuenta (RNF-37, ADR-014)**. El PO declaró la frontera técnica: generación procedural por código sobre Three.js en un WebView, sin empaquetar modelos `.glb` ni `.gltf`, y con el **presupuesto de rendimiento por encima de la fidelidad visual**. Es el primer WebView y la primera dependencia JavaScript del proyecto.
- **Decisión:** El render 3D se resuelve con **Three.js 0.160.0 en su build UMD, empaquetado como asset versionado en Git**, cargado con un `<script>` clásico desde `file:///android_asset/tree/tree.html` en un `WebView` de la plataforma. El árbol se **construye entero por código** con primitivas (`CylinderGeometry` para tronco y ramas, `IcosahedronGeometry` para la copa) y sin texturas. El estado cruza por un puente asimétrico: `evaluateJavascript` lleva **salud y etapa**, y un objeto `@JavascriptInterface` devuelve **solo** dos señales de disponibilidad del render. La representación nativa de HU-37 **se conserva de forma permanente** como fallback.

  **Alternativas descartadas:**

  | Alternativa | Por qué se descartó |
  | --- | --- |
  | Modelos `.glb` / `.gltf` empaquetados | Engrosarían el APK con geometría que el código genera gratis, y exigirían un pipeline de autoría 3D fuera del repositorio. Explícitamente prohibido por el PO. |
  | Three.js desde CDN | Rompería RNF-09 y obligaría a declarar el permiso `INTERNET`, que es justo la garantía que hoy hace imposible cualquier carga remota. |
  | Three.js como módulo ES (r161+) | Un módulo ES cargado desde `file://` choca con el origen opaco de ese esquema en varios WebView. Serviría con `WebViewAssetLoader`, pero eso añade la dependencia `androidx.webkit` y un servidor de assets interno para no ganar nada visible. El UMD de r160 —la última release que lo trae— funciona sin nada más. |
  | OpenGL ES nativo o Compose `Canvas` | Evitaría el WebView, pero significaría escribir a mano el pipeline de render, la iluminación y el control de cámara. Meses de trabajo para un elemento **puramente decorativo** que ADR-020 ya declaró aislado del dominio. |
  | `SceneView` / Filament (motor 3D nativo Android) | Añade varios megabytes de librería nativa al APK — un orden de magnitud más que los ~188 KiB que cuesta Three.js comprimido— y una dependencia de terceros de mucho más peso arquitectónico que la que se evita. |
  | Un vídeo o una secuencia de imágenes por estado de salud | Serían estados fijos y CA-38.02 exige transición **continua**; además pesaría más que el código que genera el árbol. |

- **Consecuencias:** El APK crece **192 570 bytes (188 KiB, +0,099 %)**: 699 989 bytes de assets en disco que la compresión del APK deja en 175 827. **No se añade ninguna dependencia Gradle** ni el permiso `INTERNET`, así que la imposibilidad de cargar contenido remoto la garantiza el sistema operativo y no la configuración del WebView. La homogeneidad tecnológica del proyecto se rompe por primera vez, y la ruptura queda acotada por una frontera doble: **el WebView vive en una sola pantalla** —la tarjeta de Inicio es nativa de forma permanente (RNF-01)— **y no puede salir de su archivo local**. El fallback nativo convierte al WebView en una dependencia opcional: en un Android 8 con WebView de fábrica el ejecutante ve el ícono vectorial, y eso **es** el cumplimiento de RNF-20, no un defecto. El precio asumido: `tree.js` no tiene pruebas automatizadas —el proyecto no tiene ni tendrá cadena de build JavaScript— y por eso las tres decisiones con ramas (calidad de render, soporte de WebView, enrutado del puente) se implementaron en Kotlin puro y sí están cubiertas; la geometría, la transparencia sobre distintos fabricantes y el presupuesto de 1 segundo en gama baja se verifican sobre dispositivo. La degradación es automática y unidireccional: una sonda mide los primeros fotogramas reales y baja un escalón de calidad —sombras, luego esferas de la copa, luego segmentos del tronco, luego polígonos por primitiva— porque **el presupuesto manda sobre la fidelidad**.
