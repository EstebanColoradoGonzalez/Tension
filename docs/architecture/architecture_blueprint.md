...
- **Módulo / Componente:** Entities de Room (data.local.entity)
  ...
  - **Interfaces Expuestas:** 23 data classes con anotaciones Room: `@Entity`, `@PrimaryKey`, `@ForeignKey`, `@Index`, `@ColumnInfo`.
...
- **Decisión:** `TensionDatabase.version` sube sin migración y **sin** `fallbackToDestructiveMigration()`. Aplicado a v17 (`week_day`, `daily_routine_override`), a v18 (`day_skip`), a v19 (`tree_state`, HU-37), a **v20** (`exercise_equipment`, `exercise_set.equipment_type_id` y la retirada de `exercise.equipment_type_id`, HU-39), a **v21** (`exercise_progression` reclaveada por `(exercise_id, equipment_type_id)` y la nueva `session_exercise_progression`, HU-40) y a **v22** (`muscle_zone.sort_order`, `exercise_muscle_zone.is_primary` y `plan_assignment.suggested_equipment_type_id`, HU-41 — tres columnas `NOT NULL` sin default, ninguna tabla nueva). Las migraciones históricas se conservan intactas: describen el esquema de su época. Las de `16→17`, `17→18` y `18→19`, ausentes durante tres versiones, se repusieron después de dejar la aplicación incapaz de abrir cualquier base existente — y de ahí sale la forma que toma la guarda.
